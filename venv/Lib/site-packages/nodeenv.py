    ...
    def _load(cls, configfiles, verbose=False):
        ...
        for configfile in reversed(configfiles):
            ...
            for attr, val in iteritems(vars(cls)):
                if attr.startswith("_") or not ini_file.has_option(
                    section, attr
                ):
                    continue
...
def parse_args(check=True):
    ...
    if args.config_file is None:
        ...
        if not os.path.exists(args.config_file):
            parser.error(
                "Config file '{0}' doesn't exist!".format(args.config_file)
            )
    ...
    if not args.list:
        if not args.python_virtualenv and not args.env_dir:
            parser.error(
                "You must provide a DEST_DIR or "
...
def writefile(dest, content, overwrite=True, append=False):
    ...
    if not os.path.exists(dest):
        ...
        if not overwrite:
            logger.info(
                " * File %s exists with different content; "
                " not overwriting",
                dest,
            )
            return
...
def download_node_src(node_url, src_dir, args):
    ...
    with ctx as archive:
        ...
        rexp_string = (
            r"node-v%s[^/]*/(README\.md|CHANGELOG\.md|LICENSE)" % node_ver
        )
        extract_list = [
            member
...
def build_node_from_src(env_dir, src_dir, node_src_dir, args):
    ...
    if getattr(sys.version_info, "major", sys.version_info[0]) > 2:
        ...
        env["PATH"] = "{}:{}".format(
            node_tmpbin_dir, os.environ.get("PATH", "")
        )
...
def install_node_wrapped(env_dir, src_dir, args):
    ...
    logger.info(
        " * Install %s node (%s) " % (src_type, args.node),
    ...
    if not os.path.exists(node_src_dir):
        try:
            ...
            if "arm64" in node_url:
                ...
                download_node_src(
                    node_url.replace("arm64", "x64"), src_dir, args
                )
            else:
...
def install_npm(env_dir, _src_dir, args):
    ...
    """
    logger.info(
        " * Install npm.js (%s) ... " % args.npm, extra=dict(continued=True)
    )
    env = dict(
        os.environ,
...
def install_npm_win(env_dir, src_dir, args):
    ...
    shutil.copy(
        join(src_dir, npm_ver, "bin", "npm.cmd"), join(bin_path, "npm.cmd")
    )
    shutil.copy(
        join(src_dir, npm_ver, "bin", "npm-cli.js"),
        join(bin_path, "npm-cli.js"),
    )

    if is_CYGWIN:
        shutil.copy(
            join(bin_path, "npm-cli.js"), join(env_dir, "bin", "npm-cli.js")
        )
        shutil.copytree(
            join(bin_path, "node_modules"),
...
def install_packages(env_dir, args):
    ...
    packages = [
        package.strip() for package in open(args.requirements).readlines()
    ]
...
def install_activate(env_dir, args):
    ...
    for name, content in files.items():
        ...
        content = content.replace(
            "__NODE_VIRTUAL_ENV__", os.path.abspath(env_dir)
        )
...
def get_last_lts_node_version():
    ...
    """
    return next(
        (v["version"].lstrip("v") for v in _get_versions_json() if v["lts"]),
        None,
    )
