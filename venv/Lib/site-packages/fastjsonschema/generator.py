...
class CodeGenerator:
    ...
    def generate_validation_function(self, uri, name):
        ...
        with self._resolver.resolving(uri) as definition:
            with self.l(
                "def {}(data, custom_formats={{}}, name_prefix=None):", name
            ):
    ...
    def generate_ref(self):
        ...
        with self._resolver.in_scope(self._definition["$ref"]):
            ...
            self.l(
                "{}({variable}, custom_formats, {name_arg})",
                name,
                name_arg=name_arg,
            )
    ...
    def l(self, line, *args, **kwds):
        ...
        context = dict(
            self._definition
            if self._definition and self._definition is not True
            else {},
    ...
    def exc(self, msg, *args, append_to_msg=None, rule=None):
        ...
        definition_rule = self.e(
            definition.get(rule) if isinstance(definition, dict) else None
        )
        self.l(
            msg,
            *args,
...
def serialize_regexes(patterns_dict):
    ...
    regex_patterns = (
        repr(k) + ": " + repr_regex(v) for k, v in patterns_dict.items()
    )
    return "{\n    " + ",\n    ".join(regex_patterns) + "\n}"
...
def repr_regex(regex):
    ...
    flags = " | ".join(
        f"re.{f}" for f in all_flags if regex.flags & getattr(re, f)
    )
