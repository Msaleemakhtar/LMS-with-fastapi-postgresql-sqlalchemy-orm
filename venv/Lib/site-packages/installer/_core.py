...
def _process_WHEEL_file(source: WheelSource) -> Scheme:
    ...
    if not (
        metadata["Wheel-Version"]
        and metadata["Wheel-Version"].startswith("1.")
    ):
        message = (
            "Incompatible Wheel-Version {}, only support version 1.x wheels."
        )
        raise InvalidWheelSource(
            source, message.format(metadata["Wheel-Version"])
        )
