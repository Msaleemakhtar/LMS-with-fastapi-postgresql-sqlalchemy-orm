...
class RecordEntry:
    ...
    def __init__(
        self, path: str, hash_: Optional[Hash], size: Optional[int]
    ) -> None:
    ...
    def from_elements(cls, path: str, hash_: str, size: str) -> "RecordEntry":
        ...
        if issues:
            raise InvalidRecordEntry(
                elements=(path, hash_, size), issues=issues
            )
...
def parse_record_file(rows: Iterable[str]) -> Iterator[Tuple[str, str, str]]:
    ...
    """
    reader = csv.reader(
        rows, delimiter=",", quotechar='"', lineterminator="\n"
    )
