...
class _v1ReceivePackHeader:
    def __init__(self, capabilities, old_refs, new_refs) -> None:
        ...
        self._it = self._handle_receive_pack_head(
            capabilities, old_refs, new_refs
        )
    ...
    def clone(
        ...
        try:
            ...
            if origin is None or (origin_sha and not origin_head):
                ...
                _set_origin_head(
                    target.refs, origin.encode("utf-8"), origin_head
                )
    ...
    def fetch(
        ...
        if CAPABILITY_THIN_PACK in self._fetch_capabilities:
            ...
            def commit():
                if f.tell():
                    f.seek(0)
                    target.object_store.add_thin_pack(
                        f.read, None, progress=progress
                    )
                f.close()
...
class TraditionalGitClient(GitClient):
    ...
    def send_pack(self, path, update_refs, generate_pack_data, progress=None):
        ...
        with proto:
            try:
                old_refs, server_capabilities = read_pkt_refs(
                    proto.read_pkt_seq()
                )
...
class LocalGitClient(GitClient):
    ...
    def fetch(
        self, path, target, determine_wants=None, progress=None, depth=None
    ):
        """Fetch into a target repository.
...
class SubprocessSSHVendor(SSHVendor):
    ...
    def run_command(
        ...
        if ssh_command:
            ...
            args = [
                *shlex.split(ssh_command, posix=sys.platform != "win32"),
                "-x",
            ]
        else:
            args = ["ssh", "-x"]
...
def _get_transport_and_path_from_url(url, config, operation, **kwargs):
    ...
    if parsed.scheme == "git":
        ...
        if sys.platform == "win32" or os.name == "nt":
            return default_local_git_client_cls(**kwargs), _win32_url_to_path(
                parsed
            )
        return (
...
def get_transport_and_path(
    ...
    **kwargs,
    ...
    """
    if config is not None:
        location = apply_instead_of(
            config, location, push=(operation == "push")
        )
            ...
            with open(fname, "rb") as f:
                for line in f:
                    ...
                    if (
                        ...
                        and (
                            username is None
                            or parsed_line.username == username
                        )
                    ):
