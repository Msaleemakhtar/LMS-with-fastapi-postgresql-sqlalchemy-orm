...
def hex_to_filename(path, hex):
    ...
    # as path.
    if (
        type(path) is not type(hex)
        and getattr(path, "encode", None) is not None
    ):
...
class ShaFile:
    ...
    def _parse_legacy_object_header(magic, f: BinaryIO) -> "ShaFile":
        ...
        try:
            ...
            raise ObjectFormatException(
                "Object size not an integer: %s" % exc
            ) from exc
    ...
    def set_raw_string(
        self, text: bytes, sha: Optional[ObjectID] = None
    ) -> None:
    ...
    def copy(self):
        ...
        return obj_class.from_raw_string(
            self.type_num, self.as_raw_string(), self.id
        )
...
def parse_timezone(text):
    ...
    if text[0] not in b"+-":
        raise ValueError(
            "Timezone must start with + or - ({text})".format(**vars())
        )
    sign = text[:1]
    offset = int(text[1:])
    if sign == b"-":
