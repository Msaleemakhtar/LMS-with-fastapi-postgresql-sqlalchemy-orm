...
class TimezoneTests(PorcelainTestCase):
    ...
    def fallback(self, value):
        self.put_envs(value)
        self.assertRaises(
            porcelain.TimezoneFormatError, porcelain.get_user_timezones
        )
    ...
    def test_no_envs(self):
        ...
        self.assertTupleEqual(
            (18000, local_timezone), porcelain.get_user_timezones()
        )

        self.put_envs("0 +0500")
        self.overrideEnv("GIT_AUTHOR_DATE", None)
        self.assertTupleEqual(
            (local_timezone, 18000), porcelain.get_user_timezones()
        )
...
class AddTests(PorcelainTestCase):
    def test_add_default_paths(self):
        ...
        try:
            ...
            self.assertEqual(
                {"foo", "blah", "adir", ".git"}, set(os.listdir("."))
            )
            self.assertEqual(
...
class RemoveTests(PorcelainTestCase):
    ...
    def test_remove_file_staged(self):
        ...
        try:
            ...
            porcelain.add(self.repo.path, paths=[fullpath])
            self.assertRaises(
                Exception, porcelain.rm, self.repo.path, paths=["foo"]
            )
        finally:
            os.chdir(cwd)
...
""",
    ...
    def test_tag(self):
        ...
        outstream = StringIO()
        porcelain.show(
            self.repo, objects=[b"refs/tags/tryme"], outstream=outstream
        )
        self.maxDiff = None
        self.assertMultiLineEqual(
            outstream.getvalue(),
...
class DiffTreeTests(PorcelainTestCase):
    def test_empty(self):
        ...
        outstream = BytesIO()
        porcelain.diff_tree(
            self.repo.path, c2.tree, c3.tree, outstream=outstream
        )
...
class ResetFileTests(PorcelainTestCase):
    ...
    def test_resetfile_with_dir(self):
        ...
        porcelain.reset_file(
            self.repo, os.path.join("new_dir", "foo"), target=sha
        )
...
class CheckoutTests(PorcelainTestCase):
    ...
    def test_checkout_to_branch_with_modified_file_not_present(self):
        ...
        _, nee_path = _commit_file_with_content(
            self.repo, "nee", "Good content\n"
        )
    ...
    def test_checkout_to_branch_with_multiple_files_in_sub_directory(self):
        ...
        porcelain.add(
            self.repo, paths=[sub_directory_file_1, sub_directory_file_2]
        )
        porcelain.commit(
            self.repo,
...
class StatusTests(PorcelainTestCase):
    ...
    def test_status_base(self):
        ...
        self.assertEqual(
            results.staged["add"][0], filename_add.encode("ascii")
        )
    ...
    def test_status_all(self):
        ...
        results_no_untracked = porcelain.status(
            self.repo.path, untracked_files="no"
        )
    ...
    def test_status_untracked_path(self):
        ...
        _, _, untracked = porcelain.status(
            self.repo.path, untracked_files="all"
        )
...
class BranchCreateTests(PorcelainTestCase):
    def test_branch_exists(self):
        ...
        porcelain.branch_create(self.repo, b"foo")
        self.assertRaises(
            porcelain.Error, porcelain.branch_create, self.repo, b"foo"
        )
...
class FetchTests(PorcelainTestCase):
    def test_simple(self):
        ...
        porcelain.fetch(
            target_path, "origin", outstream=outstream, errstream=errstream
        )
...
class RemoteRemoveTests(PorcelainTestCase):
    def test_remove(self):
        ...
        self.assertRaises(
            KeyError, porcelain.remote_remove, self.repo, "jelmer"
        )
        c = self.repo.get_config()
...
class CheckIgnoreTests(PorcelainTestCase):
    def test_check_ignored(self):
        ...
        with open(bar_path, "w") as f:
            f.write("BAR")
        self.assertEqual(
            ["foo"], list(porcelain.check_ignore(self.repo, [foo_path]))
        )
        self.assertEqual(
...
class UpdateHeadTests(PorcelainTestCase):
    def test_set_to_branch(self):
        ...
        self.assertEqual(
            b"ref: refs/heads/blah", self.repo.refs.read_ref(b"HEAD")
        )
    ...
    def test_set_new_branch(self):
        ...
        self.assertEqual(c1.id, self.repo.head())
        self.assertEqual(
            b"ref: refs/heads/bar", self.repo.refs.read_ref(b"HEAD")
        )
    ...
    def test_path_to_tree_path_rel(self):
        ...
        try:
            ...
            with open("baz", "w") as f:
                f.write("contents")
            self.assertEqual(
                b"bar/baz", porcelain.path_to_tree_path("..", "baz")
            )
            self.assertEqual(
                b"bar/baz",
...
class GetObjectByPathTests(PorcelainTestCase):
    def test_simple(self):
        ...
        self.assertEqual(
            b"BAR", porcelain.get_object_by_path(self.repo, "foo").data
        )
        self.assertEqual(
            b"BAR", porcelain.get_object_by_path(self.repo, b"foo").data
        )
    ...
    def test_missing(self):
        self.assertRaises(
            KeyError, porcelain.get_object_by_path, self.repo, "foo"
        )
