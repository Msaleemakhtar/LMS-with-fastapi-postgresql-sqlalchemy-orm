...
class TestPackData(PackTests):
    ...
    def test_from_file(self):
        path = os.path.join(
            self.datadir, "pack-%s.pack" % pack1_sha.decode("ascii")
        )
...
class TestPack(PackTests):
    ...
    def test_commit_obj(self):
        with self.get_pack(pack1_sha) as p:
            commit = p[commit_sha]
            self.assertEqual(
                b"James Westby <jw+debian@jameswestby.net>", commit.author
            )
...
class TestThinPack(PackTests):
    def setUp(self):
        ...
        with self.make_pack(True) as pack:
            with PackData(pack._data_path) as data:
                data.create_index(
                    self.pack_prefix + ".idx",
                    resolve_ext_ref=pack.resolve_ext_ref,
                )
    ...
    def test_get_raw(self):
        ...
        with self.make_pack(True) as p:
            self.assertEqual(
                (3, b"foo1234"), p.get_raw(self.blobs[b"foo1234"].id)
            )
...
class ReadZlibTests(TestCase):
    ...
    def test_decompress_size(self):
        ...
        self.assertRaises(
            ValueError, read_zlib_chunks, self.read, self.unpacked
        )
        self.unpacked.decomp_len = good_decomp_len - 1
        self.assertRaises(
            zlib.error, read_zlib_chunks, self.read, self.unpacked
        )
        self.unpacked.decomp_len = good_decomp_len + 1
        self.assertRaises(
            zlib.error, read_zlib_chunks, self.read, self.unpacked
...
class TestPackStreamReader(TestCase):
    ...
    def test_read_objects(self):
        ...
        self.assertEqual(
            b"".join(delta), b"".join(unpacked_delta.decomp_chunks)
        )
...
class DeltaChainIteratorTests(TestCase):
    ...
    def test_mixed_chain(self):
        ...
        # Delta resolution changed to DFS
        self.assertEntriesMatch(
            [0, 4, 2, 1, 3], entries, self.make_pack_iter(f)
        )
...
class EncodeCopyOperationTests(TestCase):
    def test_basic(self):
        ...
        self.assertEqual(
            b"\xb1\x64\xe8\x03", _encode_copy_operation(100, 1000)
        )
