...
class GitClientTests(TestCase):
    def setUp(self):
        ...
        self.client = DummyClient(
            lambda x: True, self.rin.read, self.rout.write
        )

    def test_caps(self):
        agent_cap = ("agent=dulwich/%d.%d.%d" % dulwich.__version__).encode(
            "ascii"
        )
        self.assertEqual(
            {
...
class TestGetTransportAndPathFromUrl(TestCase):
    ...
    def test_local_path(self):
        self.assertRaises(
            ValueError, get_transport_and_path_from_url, "foo.bar/baz"
        )
...
class SSHGitClientTests(TestCase):
    ...
    def test_default_command(self):
        self.assertEqual(
            b"git-upload-pack", self.client._get_cmd_path(b"upload-pack")
        )
    ...
    def test_ssh_command_precedence(self):
        ...
        test_client = SSHGitClient("git.samba.org")
        self.assertEqual(
            test_client.ssh_command, "/path/to/ssh -o Option=Value"
        )

        test_client = SSHGitClient(
            "git.samba.org", ssh_command="ssh -o Option1=Value1"
        )
...
class LocalGitClientTests(TestCase):
    ...
    def test_clone(self):
        ...
        expected[b"refs/remotes/origin/master"] = expected[
            b"refs/heads/master"
        ]
...
class HttpGitClientTests(TestCase):
    ...
    def test_url_redirect_location(self):
        ...
        for base_url in test_data.keys():
            ...
            if redirect_location:
                ...
                self.assertEqual(
                    processed_url, test_data[base_url]["redirect_url"]
                )
            else:
    ...
    def test_smart_request_content_type_with_directive_check(self):
        ...
        client = HttpGitClient(
            clone_url, pool_manager=PoolManagerMock(), config=None
        )
        self.assertTrue(
            client._smart_request("git-upload-pack", clone_url, data=None)
        )
...
class DefaultUrllib3ManagerTest(TestCase):
    def test_no_config(self):
        ...
        self.assertEqual(
            manager.connection_pool_kw["cert_reqs"], "CERT_REQUIRED"
        )
    ...
    def test_config_proxy_creds(self):
        ...
        self.assertEqual(
            ...
            {"proxy-authorization": "Basic amVsbWVyOmV4YW1wbGU="},
        )
...
class PLinkSSHVendorTests(TestCase):
    ...
    def test_run_command_password_and_privkey(self):
        ...
        for w in warnings_list:
            if (
                type(w) is type(expected_warning)
...
class RsyncUrlTests(TestCase):
    def test_simple(self):
        self.assertEqual(
            parse_rsync_url("foo:bar/path"), (None, "foo", "bar/path")
        )
        self.assertEqual(
