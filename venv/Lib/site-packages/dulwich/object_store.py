...
class PackBasedObjectStore(BaseObjectStore):
    ...
    def get_unpacked_object(
        ...
        for alternate in self.alternates:
            try:
                return alternate.get_unpacked_object(
                    hexsha, include_comp=include_comp
                )
            except KeyError:
                pass
        raise KeyError(hexsha)
    ...
    def _complete_pack(self, f, path, num_objects, indexer, progress=None):
        ...
        for i, entry in enumerate(indexer):
            if progress is not None:
                progress(
                    ("generating index: %d/%d\r" % (i, num_objects)).encode(
                        "ascii"
                    )
                )
    ...
    def add_thin_pack(self, read_all, read_some, progress=None):
        ...
        with os.fdopen(fd, "w+b") as f:
            ...
            return self._complete_pack(
                f, path, len(copier), indexer, progress=progress
            )
...
class MissingObjectFinder:
    ...
    def __init__(
        ...
        *,
        ...
        self.objects_to_send.update(
            {(w, None, None, False) for w in missing_others}
        )
    ...
    def add_todo(
        ...
        self.objects_to_send.update(
            [e for e in entries if e[0] not in self.sha_done]
        )
    ...
    def __next__(self) -> Tuple[bytes, Optional[PackHint]]:
        ...
        if not leaf:
            ...
            if isinstance(o, Commit):
                ...
                self.add_todo(
                    [(o.object[1], None, o.object[0].type_num, False)]
                )
