        ...
        if stage == Stage.NORMAL:
            ...
            if isinstance(existing, IndexEntry):
                raise AssertionError(
                    "Non-conflicted entry for %r exists" % entry.name
                )
    ...
    for key in sorted(entries):
        ...
        if isinstance(value, ConflictedIndexEntry):
            if value.ancestor is not None:
                entries_list.append(
                    value.ancestor.serialize(
                        key, Stage.MERGE_CONFLICT_ANCESTOR
                    )
                )
...
class Index:
    ...
    def __getitem__(
        self, key: bytes
    ) -> Union[IndexEntry, ConflictedIndexEntry]:
...
def validate_path(
    path: bytes, element_validator=validate_path_element_default
) -> bool:
