...
class FileSystemBackend(Backend):
    ...
    def open_repository(self, path):
        ...
        if not normcase_abspath.startswith(normcase_root):
            raise NotGitRepository(
                f"Path {path!r} not inside root {self.root!r}"
            )
        return Repo(abspath)
...
def _all_wants_satisfied(store: ObjectContainer, haves, wants):
    ...
    if haves:
        ...
        earliest = min(
            [h.commit_time for h in have_objs if isinstance(h, Commit)]
        )
    else:
        earliest = 0
    for want in wants:
...
class TCPGitServer(socketserver.TCPServer):
    ...
    def __init__(
        self, backend, listen_addr, port=TCP_GIT_PORT, handlers=None
    ) -> None:
