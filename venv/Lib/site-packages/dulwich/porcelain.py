...
def parse_timezone_format(tz_str):
    ...
    if re.match(internal_format_pattern, tz_str):
        try:
            tz_internal = parse_timezone(
                tz_str.split(" ")[1].encode(DEFAULT_ENCODING)
            )
...
def get_user_timezones():
    ...
    if os.environ.get("GIT_COMMITTER_DATE"):
        commit_timezone = parse_timezone_format(
        ...
        if config.has_section(section):
            ...
            try:
                branch_create(
                    repo,
                    checkout_target,
                    (LOCAL_REMOTE_PREFIX + target).decode(),
                )
            except Error:
                pass
...
def checkout_branch(repo, target: Union[bytes, str], force: bool = False):
    ...
    if force:
        ...
        while index < len(changes):
            ...
            try:
                ...
                try:
                    target_tree.lookup_path(
                        repo.object_store.__getitem__, change
                    )
                    index += 1
                except KeyError:
                    raise CheckoutError(
    ...
    repo_index = repo.open_index()
    for change in repo_index.changes_from_tree(
        repo.object_store, current_tree.id
    ):
