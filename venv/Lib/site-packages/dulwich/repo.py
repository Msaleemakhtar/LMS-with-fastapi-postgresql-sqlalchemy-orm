...
class BaseRepo:
    ...
    def __init__(
        self, object_store: PackBasedObjectStore, refs: RefsContainer
    ) -> None:
        """Open a repository.
    ...
    def find_missing_objects(
        ...
        shallows: FrozenSet[ObjectID] = getattr(
            graph_walker, "shallow", frozenset()
        )
    ...
    def get_parents(
        self, sha: bytes, commit: Optional[Commit] = None
    ) -> List[bytes]:
    ...
    def get_config_stack(self) -> "StackedConfig":
        ...
        if local_config.get_boolean(
            (b"extensions",), b"worktreeconfig", False
        ):
    ...
    def get_walker(
        self, include: Optional[List[bytes]] = None, *args, **kwargs
    ):
...
class Repo(BaseRepo):
    ...
    def _init_maybe_bare(
        ...
        ret.refs.set_symbolic_ref(
            b"HEAD", LOCAL_BRANCH_PREFIX + default_branch
        )
