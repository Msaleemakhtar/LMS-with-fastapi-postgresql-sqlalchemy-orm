...
class PackIndex:
    ...
    def object_index(self, sha: bytes) -> int:
        warnings.warn(
            "Please use object_offset instead",
            DeprecationWarning,
            stacklevel=2,
        )
...
class MemoryPackIndex(PackIndex):
    ...
    def for_pack(cls, pack):
        return MemoryPackIndex(
            pack.sorted_entries(), pack.calculate_checksum()
        )
...
class PackIndex1(FilePackIndex):
    ...
    def __init__(
        self, filename: str, file=None, contents=None, size=None
    ) -> None:
...
class PackStreamReader:
    ...
    def __init__(
        self, read_all, read_some=None, zlib_bufsize=_ZLIB_BUFSIZE
    ) -> None:
    ...
    def verify(self, progress=None):
        ...
        for i, unpacked in enumerate(self.read_objects()):
            ...
            if progress is not None:
                progress(
                    ("copying pack entries: %d/%d\r" % (i, len(self))).encode(
                        "ascii"
                    )
                )
        if progress is not None:
...
class PackData:
    ...
    def create_index(
        self, filename, progress=None, version=2, resolve_ext_ref=None
    ):
...
def write_pack(
    filename,
    objects: Union[
        Sequence[ShaFile], Sequence[Tuple[ShaFile, Optional[bytes]]]
    ],
    *,
    ...
    *,
    ...
    for i, unpacked in enumerate(
        ...
        if progress is not None and i % 1000 == 0:
            progress(
                (
                    "checking for reusable deltas: %d/%d\r"
                    % (i, len(object_ids))
                ).encode("utf-8")
            )
...
def sort_objects_for_delta(
    objects: Union[
        Iterator[ShaFile], Iterator[Tuple[ShaFile, Optional[PackHint]]]
    ]
) -> Iterator[ShaFile]:
    magic = []
    ...
    def _pack_data_chunks(
        ...
        *,
        ...
        for i, unpacked in enumerate(records):
            ...
            raw: Union[
                List[bytes], Tuple[int, List[bytes]], Tuple[bytes, List[bytes]]
            ]
...
class Pack:
    ...
    def get_ref(
        self, sha: bytes
    ) -> Tuple[Optional[int], int, OldUnpackedObject]:
    ...
    def get_unpacked_object(
        ...
        *,
        ...
        """
        offset = self.index.object_offset(sha)
        unpacked = self.data.get_unpacked_object_at(
            offset, include_comp=include_comp
        )
        if unpacked.pack_type_num == OFS_DELTA and convert_ofs_delta:
            assert isinstance(unpacked.delta_base, int)
            unpacked.delta_base = self.index.object_sha1(
                offset - unpacked.delta_base
            )
            unpacked.pack_type_num = REF_DELTA
        return unpacked
    ...
    *,
    ...
    for i, object_id in enumerate(object_ids):
        if progress is not None:
            progress(
                (
                    "writing extra base objects: %d/%d\r"
                    % (i, len(object_ids))
                ).encode("ascii")
            )
