    ...
    def visit_simple_stmt(self, node: Node) -> Iterator[Line]:
        ...
        if node.parent and node.parent.type in STATEMENT:
            ...
            if (
                not (
                    self.mode.is_pyi
                    or Preview.dummy_implementations in self.mode
                )
                or not node.parent
        ...
        if hugged_opening_leaves and inner_body_leaves:
            ...
            if (
                ...
                if is_line_short_enough(
                    inner_body,
                    mode=replace(line.mode, line_length=line_length),
                ):
            ...
            if should_hug:
                ...
                body = (
                    inner_body  # No need to re-calculate the body again later.
                )
...
def _prefer_split_rhs_oop_over_rhs(
    ...
    # the body
    rhs_head_equal_count = [leaf.type for leaf in rhs.head.leaves].count(
        token.EQUAL
    )
    rhs_oop_head_equal_count = [
        leaf.type for leaf in rhs_oop.head.leaves
    ].count(token.EQUAL)
    if (
        rhs_head_equal_count > 1
        and rhs_head_equal_count > rhs_oop_head_equal_count
    ):
        return False
