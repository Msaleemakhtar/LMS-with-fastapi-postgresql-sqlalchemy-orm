...
class StringParenStripper(StringTransformer):
    ...
    def do_transform(
        ...
        if string_and_rpar_indices:
            yield Ok(
                self._transform_to_new_line(line, string_and_rpar_indices)
            )
        else:
            yield Err(
                CannotTransform(
                    "All string groups have comments attached to them."
                )
            )
    ...
    def _transform_to_new_line(
        ...
        for idx in sorted(string_and_rpar_indices):
            ...
            if leaf.type == token.STRING:
                ...
                new_line.comments.setdefault(id(string_leaf), []).extend(
                    old_comments
                )
            else:
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
        ...
        while more_splits_should_be_made():
            ...
            if (
                ...
                and (
                    ...
                    or next_value
                    != self._normalize_f_string(next_value, prefix)
                )
            ):
    ...
    def _get_break_idx(self, string: str, max_break_idx: int) -> Optional[int]:
        ...
        def passes_all_checks(i: Index) -> bool:
            ...
            is_space = string[i] == " "
            is_split_safe = (
                is_valid_index(i - 1) and string[i - 1] in SPLIT_SAFE_CHARS
            )
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_splitter_match(self, line: Line) -> TMatchResult:
        ...
        if string_idx is not None:
            ...
            if not any(
                char == " " or char in SPLIT_SAFE_CHARS
    ...
    def do_transform(
        ...
        if is_valid_index(string_idx + 1):
            ...
            if old_parens_exist:
                ...
                if (
                    opening_bracket is not None
                    and opening_bracket in left_leaves
                ):
