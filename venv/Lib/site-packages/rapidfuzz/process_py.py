...
def _get_scorer_flags_py(
    scorer: Any, scorer_kwargs: dict[str, Any]
) -> tuple[int, int]:
...
def _is_symmetric(
    scorer: Callable[..., int | float], scorer_kwargs: dict[str, Any]
) -> bool:
    params = getattr(scorer, "_RF_ScorerPy", None)
    if params is not None:
        flags = params["get_scorer_flags"](**scorer_kwargs)
    ...
    *,
    ...
    if queries is choices and _is_symmetric(scorer, scorer_kwargs):
        for i, query in enumerate(proc_choices):
            score = (
                scorer(
                    query, query, score_cutoff=score_cutoff, **scorer_kwargs
                )
                * score_multiplier
            )
