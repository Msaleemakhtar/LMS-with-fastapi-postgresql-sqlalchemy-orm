...
class Connection(metaclass=ConnectionMeta):
    ...
    def __init__(
        ...
        self._server_version = serverversion.split_server_version_string(
            ver_string
        )
    ...
    async def _get_statement(
        ...
        *,
        ...
        if use_cache:
            ...
            use_cache = self._stmt_cache_enabled and (
        ...
        if need_reprepare or (
            not statement.name and not self._stmt_cache_enabled
        ):
    ...
    def cursor(
        self, query, *args, prefetch=None, timeout=None, record_class=None
    ):
    ...
    async def set_type_codec(
        self, typename, *, schema="public", encoder, decoder, format="text"
    ):
    ...
    async def set_builtin_type_codec(
        self, typename, *, schema="public", codec_name, format=None
    ):
        ...
        if not introspection.is_scalar_type(typeinfo):
            raise exceptions.InterfaceError(
                "cannot alias non-scalar type {}.{}".format(schema, typename)
            )
    ...
    async def _cancel(self, waiter):
        try:
            ...
            self._cancellations.discard(asyncio.current_task(self._loop))
            if not waiter.done():
    ...
    def _on_release(self, stacklevel=1):
        ...
        self._check_listeners(self._log_listeners, "log")
...
def _check_record_class(record_class):
    if record_class is protocol.Record:
        pass
    elif isinstance(record_class, type) and issubclass(
        record_class, protocol.Record
    ):
        if (
