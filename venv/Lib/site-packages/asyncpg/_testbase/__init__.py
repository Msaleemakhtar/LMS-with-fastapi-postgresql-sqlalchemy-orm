                ...
                if timeout:
                    ...
                    try:
                        ...
                        raise self.failureException(
                            "test timed out after {} seconds".format(timeout)
                        ) from None
                else:
                    self.loop.run_until_complete(coro)
...
def _start_cluster(
    ClusterCls, cluster_kwargs, server_settings, initdb_options=None
):
...
class ClusterTestCase(TestCase):
    ...
    def new_cluster(cls, ClusterCls, *, cluster_kwargs={}, initdb_options={}):
        cluster = _init_cluster(
            ClusterCls, cluster_kwargs, _get_initdb_options(initdb_options)
        )
...
class HotStandbyTestCase(ClusterTestCase):
    ...
    def setup_cluster(cls):
        ...
        try:
            ...
            cls.start_cluster(
                cls.standby_cluster, server_settings={"hot_standby": True}
            )
