...
class PostgresMessageMeta(type):
    ...
    def __new__(mcls, name, bases, dct):
        ...
        if _is_asyncpg_class(cls):
            ...
            if hasattr(mod, name):
                raise RuntimeError(
                    "exception class redefinition: {}".format(name)
                )
        ...
        if code is not None:
            ...
            if existing is not None:
                raise TypeError(
                    "{} has duplicate SQLSTATE code, which is"
                    "already defined by {}".format(name, existing.__name__)
                )
...
class PostgresMessage(metaclass=PostgresMessageMeta):
    ...
    def _make_constructor(cls, fields, query=None):
        ...
        is_prepared_stmt_error = exccls.__name__ in (
...
class InterfaceError(InterfaceMessage, Exception):
    ...
    def with_msg(self, msg):
        return type(self)(
            ...
            detail=self.detail,
            hint=self.hint,
        ).with_traceback(self.__traceback__)
...
class PostgresLogMessage(PostgresMessage):
    ...
    def __setattr__(self, name, val):
        raise TypeError(
            "instances of {} are immutable".format(type(self).__name__)
        )
