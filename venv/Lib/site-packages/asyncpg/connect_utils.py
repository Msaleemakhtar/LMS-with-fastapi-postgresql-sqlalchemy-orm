...
async def _create_ssl_connection(
    protocol_factory, host, port, *, loop, ssl_context, ssl_is_advisory=False
):
...
async def __connect_addr(
    ...
    try:
        ...
        if retry and (
            params.sslmode == SSLMode.allow
            and not pr.is_ssl
            or params.sslmode == SSLMode.prefer
            and pr.is_ssl
        ):
            # Trigger retry when:
    ...
    *, loop, addr, params: _ConnectionParameters, backend_pid, backend_secret
    ...
    if isinstance(addr, str):
        ...
        if params.ssl and params.sslmode != SSLMode.allow:
            ...
            tr, pr = await loop.create_connection(CancelProto, *addr)
