...
class Cluster:
    def __init__(self, data_dir, *, pg_config_path=None):
        ...
        self._pg_bin_dir = os.environ.get("PGINSTALLATION") or os.environ.get(
            "PGBIN"
        )
        self._pg_ctl = None
    ...
    def reset_wal(self, *, oid=None, xid=None):
        ...
        if status == "running":
            raise ClusterError("cannot modify WAL status: cluster is running")
        ...
        process = subprocess.run(
            [reset_wal] + opts, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def reset_hba(self):
        ...
        try:
            ...
            raise ClusterError(
                "cannot modify HBA records: {}".format(e)
            ) from e
    ...
    def _connection_addr_from_pidfile(self):
        ...
        return {"host": host_str, "port": portnum}
    ...
    def _run_pg_config(self, pg_config_path):
        ...
        if process.returncode != 0:
            raise ClusterError(
                "pg_config exited with status {:d}: {}".format(
                    process.returncode, stderr
                )
            )
        else:
    ...
    def _find_pg_config(self, pg_config_path):
        if pg_config_path is None:
            pg_install = os.environ.get("PGINSTALLATION") or os.environ.get(
                "PGBIN"
            )
            if pg_install:
        ...
        if not os.path.isfile(pg_config_path):
            raise ClusterError(
                "{!r} is not an executable".format(pg_config_path)
            )
    ...
    def __init__(
        ...
        *,
        ...
        self._data_dir = tempfile.mkdtemp(
            suffix=data_dir_suffix, prefix=data_dir_prefix, dir=data_dir_parent
        )
