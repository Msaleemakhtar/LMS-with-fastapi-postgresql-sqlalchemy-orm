...
from crashtest.contracts.has_solutions_for_exception import (
...
class SolutionProviderRepository(BaseSolutionProviderRepository):
    ...
    def get_solutions_for_exception(
        ...
        for solution_provider_class in self._solution_providers:
            if not issubclass(
                solution_provider_class, HasSolutionsForException
            ):
                continue

            solution_provider: HasSolutionsForException = (
                solution_provider_class()
            )
