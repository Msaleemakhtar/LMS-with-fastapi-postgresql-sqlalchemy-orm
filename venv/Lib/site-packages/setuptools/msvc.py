...
def _msvc14_find_vc2017():
    ...
    for component in suitable_components:
        ...
        with contextlib.suppress(
            CalledProcessError, OSError, UnicodeDecodeError
        ):
            path = (
    ...
    def UniversalCRTSdkDir(self):
        ...
        for ver in vers:
            sdkdir = self.ri.lookup(
                self.ri.windows_kits_roots, "kitsroot%s" % ver
            )
            if sdkdir:
                return sdkdir or ""
...
class EnvironmentInfo:
    ...
    def FxTools(self):
        ...
        if include32:
            tools += [
                join(si.FrameworkDir32, ver) for ver in si.FrameworkVersion32
            ]
        if include64:
            tools += [
                join(si.FrameworkDir64, ver) for ver in si.FrameworkVersion64
            ]
        return tools
