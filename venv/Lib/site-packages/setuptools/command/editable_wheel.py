...
class editable_wheel(Command):
    ...
    def finalize_options(self):
        ...
        self.dist_dir = Path(
            self.dist_dir or os.path.join(self.project_dir, "dist")
        )
    ...
    def _install_namespaces(self, installation_dir, pth_prefix):
        ...
        src_root = Path(
            self.project_dir, self.package_dir.get("", ".")
        ).resolve()
    ...
    def _find_egg_info_dir(self) -> Optional[str]:
        parent_dir = (
            Path(self.dist_info_dir).parent if self.dist_info_dir else Path()
        )
    ...
    def _configure_build(
        ...
        egg_info = dist.reinitialize_command(
    ...
    def _select_strategy(
        ...
        if (
            ...
            return _StaticPth(
                self.distribution, name, [Path(project_dir, src_dir)]
            )
...
class _StaticPth:
    def __init__(
        self, dist: Distribution, name: str, path_entries: List[Path]
    ):
        self.dist = dist
        self.name = name
...
class _TopLevelFinder:
    ...
    def __call__(
        ...
        top_level = chain(
            _find_packages(self.dist), _find_top_level_modules(self.dist)
        )
...
def _can_symlink_files(base_dir: Path) -> bool:
    with TemporaryDirectory(dir=str(base_dir.resolve())) as tmp:
        ...
        with suppress(AttributeError, NotImplementedError, OSError):
            ...
            if (
                path2.is_symlink()
                and path2.read_text(encoding="utf-8") == "file1"
            ):
                return True
...
def _simple_layout(
    ...
    parent = os.path.commonpath(
        [_parent_path(k, v) for k, v in layout.items()]
    )
    return all(
...
class _NamespaceInstaller(namespaces.Installer):
    def __init__(
        self, distribution, installation_dir, editable_name, src_root
    ):
    ...
    def _get_nspkg_file(self):
        """Installation target."""
        return os.path.join(
            self.installation_dir, self.editable_name + self.nspkg_ext
        )
