...
class MacroExpander:
    ...
    def load_macros(self, version):
        self.set_macro(
            "VCInstallDir", self.vsbase + r"\Setup\VC", "productdir"
        )
        self.set_macro(
            "VSInstallDir", self.vsbase + r"\Setup\VS", "productdir"
        )
        self.set_macro("FrameworkDir", NET_BASE, "installroot")
        try:
            if version >= 8.0:
                self.set_macro(
                    "FrameworkSDKDir", NET_BASE, "sdkinstallrootv2.0"
                )
            else:
...
you can try compiling with MingW32, by passing "-c mingw32" to setup.py."""
        ...
        if version >= 9.0:
            ...
            self.set_macro(
                "WindowsSdkDir", WINSDK_BASE, "currentinstallfolder"
            )
        else:
...
class MSVCCompiler(CCompiler):
    ...
    src_extensions = (
        _c_extensions + _cpp_extensions + _rc_extensions + _mc_extensions
    )
    res_extension = ".res"
    obj_extension = ".obj"
