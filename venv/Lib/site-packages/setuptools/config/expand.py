...
def read_files(
    filepaths: Union[str, bytes, Iterable[_Path]], root_dir=None
) -> str:
    ...
    _filepaths = (
        os.path.join(root_dir, path) for path in always_iterable(filepaths)
    )
    return "\n".join(
        _read_file(path)
...
def _assert_local(filepath: _Path, root_dir: str):
    if (
        ...
        not in Path(os.path.abspath(filepath)).parents
    ):
...
def read_attr(
    ...
    _parent_path, path, module_name = _find_module(
        module_name, package_dir, root_dir
    )
...
def _load_spec(spec: ModuleSpec, module_name: str) -> ModuleType:
    ...
    sys.modules[
        name
    ] = module  # cache (it also ensures `==` works on loaded items)
...
def cmdclass(
    ...
    """
    return {
        k: resolve_class(v, package_dir, root_dir) for k, v in values.items()
    }
...
def find_packages(
    ...
    **kwargs,
    ...
    if len(search) == 1 and all(
        not _same_path(search[0], x) for x in (".", root_dir)
    ):
