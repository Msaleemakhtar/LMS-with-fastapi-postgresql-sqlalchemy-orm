...
def apply(
    dist: "Distribution", config: dict, filename: _Path
) -> "Distribution":
...
def _license(dist: "Distribution", val: dict, root_dir: _Path):
    ...
    if "file" in val:
        _set_config(
            dist, "license", expand.read_files([val["file"]], root_dir)
        )
...
def _people(
    dist: "Distribution", val: List[dict], _root_dir: _Path, kind: str
):
    field = []
    email_field = []
    for person in val:
...
def _copy_command_options(
    pyproject: dict, dist: "Distribution", filename: _Path
):
...
def _normalise_cmd_options(
    desc: List[Tuple[str, Optional[str], str]]
) -> Set[str]:
    return {
