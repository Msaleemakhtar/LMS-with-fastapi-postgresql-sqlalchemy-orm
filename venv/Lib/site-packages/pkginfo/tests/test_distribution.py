...
class Test__must_decode(unittest.TestCase):
    ...
    def test_w_bytes_latin1(self):
        TO_ENCODE = "\u00C9"  # capital E w/ acute accent
    ...
    def test_w_unicode(self):
        ARG = "\u00C9"  # capital E w/ acute accent
...
class DistributionTests(unittest.TestCase):
    ...
    def test_parse_Metadata_Version_2_2(self):
        ...
        self.assertEqual(list(dist), [x[1] for x in HEADER_ATTRS_2_2])
    ...
    def test_parse_Supported_Platform_multiple(self):
        dist = self._makeOne()
        dist.parse(
            "Supported-Platform: i386-win32\n" "Supported-Platform: RedHat 7.2"
        )
        self.assertEqual(
            list(dist.supported_platforms), ["i386-win32", "RedHat 7.2"]
        )
    ...
    def test_parse_Classifier_multiple(self):
        dist = self._makeOne("1.1")
        dist.parse(
            "Classifier: Some :: Silly Thing\n" "Classifier: Or :: Other"
        )
        self.assertEqual(
            list(dist.classifiers), ["Some :: Silly Thing", "Or :: Other"]
        )

    def test_parse_Download_URL(self):
        dist = self._makeOne("1.1")
        dist.parse(
            "Download-URL: " "http://example.com/package/mypackage-0.1.zip"
        )
        self.assertEqual(
            dist.download_url, "http://example.com/package/mypackage-0.1.zip"
        )
    ...
    def test_parse_Requires_Dist_single_w_version(self):
        ...
        self.assertEqual(
            list(dist.requires_dist), ["SpanishInquisition (>=1.3)"]
        )

    def test_parse_Requires_Dist_single_w_env_marker(self):
        dist = self._makeOne("1.2")
        dist.parse(
            "Requires-Dist: SpanishInquisition; " "python_version == '1.4'"
        )
        self.assertEqual(
    ...
    def test_parse_Requires_Dist_multiple(self):
        dist = self._makeOne("1.2")
        dist.parse(
            "Requires-Dist: SpanishInquisition\n"
    ...
    def test_parse_Provides_Dist_single_w_env_marker(self):
        ...
        self.assertEqual(
            list(dist.provides_dist), ["SillyWalks; sys.platform == 'os2'"]
        )

    def test_parse_Provides_Dist_multiple(self):
        dist = self._makeOne("1.2")
        dist.parse(
            "Provides-Dist: SillyWalks\n"
    ...
    def test_parse_Obsoletes_Dist_single_w_env_marker(self):
        ...
        self.assertEqual(
            list(dist.obsoletes_dist), ["SillyWalks; sys.platform == 'os2'"]
        )

    def test_parse_Obsoletes_Dist_multiple(self):
        dist = self._makeOne("1.2")
        dist.parse(
            "Obsoletes-Dist: kniggits\n"
    ...
    def test_parse_Project_URL_multiple(self):
        dist = self._makeOne("1.2")
        dist.parse(
            "Project-URL: Bug tracker, http://bugs.example.com/grail\n"
    ...
    def test_parse_Dynamic_multiple(self):
        ...
        self.assertEqual(
            list(dist.dynamic), ["Platforms", "Supported-Platforms"]
        )
