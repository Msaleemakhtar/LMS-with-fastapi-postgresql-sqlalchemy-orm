...
class ArgvInput(Input):
    ...
    def first_argument(self) -> str | None:
        ...
        for i, token in enumerate(self._tokens):
            if token.startswith("-"):
                ...
                if not (
                    name in self._options
                    or self._definition.has_shortcut(name)
                ):
                    # noop
                    continue
                ...
                if (
                    name in self._options
                    and self._tokens[i + 1] == self._options[name]
                ):
                    is_option = True
    ...
    def _parse_short_option(self, token: str) -> None:
        ...
        if len(name) > 1:
            ...
            if (
                ...
                and self._definition.option_for_shortcut(
                    shortcut
                ).accepts_value()
            ):
        ...
        for i in range(length):
            ...
            if not self._definition.has_shortcut(shortcut):
                raise CleoRuntimeError(
                    f'The option "{name[i]}" does not exist'
                )
    ...
    def _parse_argument(self, token: str) -> None:
        ...
        if self._definition.has_argument(next_argument):
            ...
            self._arguments[argument.name] = (
                [token] if argument.is_list() else token
            )
        ...
        # Unexpected argument
            ...
            if all_arguments:
                ...
                if command_name:
                    ...
                    message = (
                        f"Too many arguments, expected arguments {all_names}"
                    )
            elif command_name:
                message = (
    ...
    def _add_short_option(self, shortcut: str, value: Any) -> None:
        if not self._definition.has_shortcut(shortcut):
            raise CleoNoSuchOptionError(
                f'The option "-{shortcut}" does not exist'
            )
    ...
    def _add_long_option(self, name: str, value: Any) -> None:
        if not self._definition.has_option(name):
            raise CleoNoSuchOptionError(
                f'The option "--{name}" does not exist'
            )
        ...
        if not (value is None or option.accepts_value()):
            raise CleoRuntimeError(
                f'The "--{name}" option does not accept a value'
            )
        ...
        if value is None:
            if option.requires_value():
                raise CleoRuntimeError(
                    f'The "--{name}" option requires a value'
                )
