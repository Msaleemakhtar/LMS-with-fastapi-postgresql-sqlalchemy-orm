...
class Application:
    ...
    def __init__(self, name: str = "console", version: str = "") -> None:
        ...
        self._solution_provider_repository: SolutionProviderRepository | None = (
            None
        )
    ...
    def has(self, name: str) -> bool:
        ...
        return bool(
            self._command_loader.has(name)
            and self.add(self._command_loader.get(name))
        )
        ...
        if self._command_loader:
            for name in self._command_loader.names:
                if (
                    ...
                    and namespace
                    == self.extract_namespace(name, name.count(" ") + 1)
                    and self.has(name)
                ):
                    commands[name] = self.get(name)
    ...
    def _run(self, io: IO) -> int:
        ...
        if io.input.has_parameter_option(["--help", "-h"], True):
            if not name:
                name = "help"
                io.set_input(
                    ArgvInput(["console", "help", self._default_command])
                )
            else:
    ...
    def render_error(self, error: Exception, io: IO) -> None:
        ...
        trace = ExceptionTrace(
            error,
            solution_provider_repository=self._solution_provider_repository,
        )
    ...
    def _default_definition(self) -> Definition:
        return Definition(
            [
                ...
                Option(
                    "--no-ansi", flag=True, description="Disable ANSI output."
                ),
                Option(
                    "--no-interaction",
                    "-n",
    ...
    def _extract_all_namespaces(self, name: str) -> list[str]:
        ...
        for part in parts:
            namespaces.append(
                namespaces[-1] + " " + part if namespaces else part
            )
