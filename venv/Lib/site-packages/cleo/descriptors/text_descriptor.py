...
class TextDescriptor(Descriptor):
    ...
    def _describe_definition(
        self, definition: Definition, **options: Any
    ) -> None:
        arguments = definition.arguments
        definition_options = definition.options
        total_width = self._calculate_total_width_for_options(
            definition_options
        )
    ...
    def _describe_application(
        self, application: Application, **options: Any
    ) -> None:
        from cleo.descriptors.application_description import (
            ApplicationDescription,
        )

        described_namespace = options.get("namespace")
        description = ApplicationDescription(
            application, namespace=described_namespace
        )
        ...
        self._describe_definition(
            Definition(application.definition.options), **options
        )
        ...
        for namespace in namespaces.values():
            namespace["commands"] = [
                c for c in namespace["commands"] if c in commands
            ]
    ...
    def _calculate_total_width_for_options(self, options: list[Option]) -> int:
        ...
        for option in options:
            name_length = (
                1 + max(len(option.shortcut or ""), 1) + 4 + len(option.name)
            )
