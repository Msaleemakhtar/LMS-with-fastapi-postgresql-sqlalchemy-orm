...
"""
    ...
    def render_bash(self) -> str:
        ...
        opts = [
            f"--{opt.name}"
            for opt in sorted(
                self.application.definition.options, key=lambda o: o.name
            )
        ]
        ...
        cmds_opts = []
        for cmd in sorted(
            self.application.all().values(), key=lambda c: c.name or ""
        ):
            if cmd.hidden or not (cmd.enabled and cmd.name):
                continue
            command_name = (
                shell_quote(cmd.name) if " " in cmd.name else cmd.name
            )
        ...
        return TEMPLATES["bash"] % {
            ...
            "compdefs": "\n".join(
                f"complete -o default -F {function} {alias}"
    ...
    def render_zsh(self) -> str:
        ...
        for cmd in sorted(
            ...
            cmds.append(
                self._zsh_describe(command_name, sanitize(cmd.description))
            )
            options = " ".join(
                self._zsh_describe(f"--{opt.name}", sanitize(opt.description))
                for opt in sorted(cmd.definition.options, key=lambda o: o.name)
    ...
    def render_fish(self) -> str:
        ...
        for cmd in sorted(
            ...
            if len(cmd_path) == 1:
                ...
                subcmds = [
                    name.split(" ")[-1]
                    for name in self.application.all(namespace)
                ]
                cmds.append(
            ...
            cmds_opts += [
                ...
                *[
                    ...
                    f"-l {opt.name} -d '{sanitize(opt.description)}'"
                    for opt in sorted(
                        cmd.definition.options, key=lambda o: o.name
                    )
                ],
                "",  # newline
            ]
    ...
    def _generate_function_name(
        self, script_name: str, script_path: str
    ) -> str:
