...
class BaseCommand:
    ...
    def processed_help(self) -> str:
        ...
        is_single_command = (
            self._application and self._application.is_single_command()
        )
    ...
    def run(self, io: IO) -> int:
        ...
        if (
            io.input.has_argument("command")
            and io.input.argument("command") is None
        ):
    ...
    def merge_application_definition(self, merge_args: bool = True) -> None:
        ...
        if merge_args:
            self._full_definition.set_arguments(
                self._application.definition.arguments
            )
    ...
    def synopsis(self, short: bool = False) -> str:
        ...
        if key not in self._synopsis:
            self._synopsis[
                key
            ] = f"{self.name} {self.definition.synopsis(short)}"
