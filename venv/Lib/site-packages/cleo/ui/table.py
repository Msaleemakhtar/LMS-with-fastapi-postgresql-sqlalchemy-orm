...
class Table:
    ...
    def set_column_style(
        self, column_index: int, style: str | TableStyle
    ) -> Table:
            ...
            for i, header in enumerate(headers):
                ...
                for row in self._rows:
                    ...
                    if len(row) > i:
                        ...
                        isinstance(rows_i[0], TableCell)
                        and rows_i[0].colspan >= 2
                    ):
                        # There is a title
                        pass
                    else:
    ...
    def _render_row_separator(
        ...
        if title is not None:
            ...
            if title_length > limit:
                title_length = limit
                format_length = len(
                    self._io.remove_format(title_format.format(""))
                )
    ...
    def _render_row(
        ...
        row: list[str],
        cell_format: str,
        first_cell_format: str | None = None,
    ) -> None:
        """
        Renders table row.
    ...
    def _render_cell(self, row: Row, column: int, cell_format: str) -> str:
        ...
        pad = style.pad
        if isinstance(cell, TableCell) and isinstance(
    ...
    def _build_table_rows(self, rows: Rows) -> Iterator[Row | TableSeparator]:
        ...
        while row_key < len(rows):
            ...
            for column, cell in enumerate(rows[row_key]):
                ...
                if (
                    column in self._column_max_widths
                    and self._column_max_widths[column]
        ...
        for cell_key, cell in enumerate(row):
            if isinstance(cell, TableCell) and cell.colspan > 1:
                ...
                columns = [
                    ...
                    if column
                    not in range(cell_key + 1, cell_key + cell.colspan)
                ]
