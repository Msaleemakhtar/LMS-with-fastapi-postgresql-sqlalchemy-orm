...
class Highlighter:
        ...
        for token_info in tokens:
            ...
            if lineno < end[0]:
                ...
                for token_line in token_lines[1:-1]:
                    lines.append(
                        f"<{self._theme[current_type]}>{token_line}</>"
                    )
    ...
    def line_numbers(
        self, lines: list[str], mark_line: int | None = None
    ) -> list[str]:
...
class ExceptionTrace:
    ...
    _FRAME_SNIPPET_CACHE: ClassVar[
        dict[tuple[Frame, int, int], list[str]]
    ] = {}
    ...
    def _render_exception(
        self, io: IO | Output, exception: BaseException
    ) -> None:
        ...
        self._render_line(
            io, f"<error>{inspector.exception_name}</error>", True
        )
        io.write_line("")
        exception_message = (
            Formatter()
            .format(inspector.exception_message)
            .replace("\n", "\n  ")
        )
    ...
    def _render_snippet(self, io: IO | Output, frame: Frame) -> None:
        ...
        ).code_snippet(frame.file_content, frame.lineno, 4, 4)
    ...
    def _render_solution(self, io: IO | Output, exception: Exception) -> None:
        ...
        solutions = (
            self._solution_provider_repository.get_solutions_for_exception(
                exception
            )
        )
        ...
        if io.is_very_verbose() and remaining_frames_length:
            ...
            for collection in frame_collections:
                if collection.is_repeated():
                    if len(collection) > 1:
                        frames_message = (
                            f"<fg=yellow>{len(collection)}</> frames"
                        )
                    else:
