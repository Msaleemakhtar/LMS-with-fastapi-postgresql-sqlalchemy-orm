...
class FieldInfo(_repr.Representation):
    ...
    def from_annotated_attribute(
        cls, annotation: type[Any], default: Any
    ) -> FieldInfo:
        ...
        if isinstance(default, cls):
            default.annotation, annotation_metadata = cls._extract_metadata(
                annotation
            )
            ...
            if annotation is dataclasses.InitVar:
                if sys.version_info < (3, 8):
                    raise RuntimeError(
                        "InitVar is not supported in Python 3.7 as type information is lost"
                    )
            ...
            (
                ...
                annotation_metadata,
            ) = cls._extract_metadata(annotation)
        ...
        *field_infos: FieldInfo, **overrides: Any
        ...
        for field_info in field_infos:
            flattened_field_infos.extend(
                x for x in field_info.metadata if isinstance(x, FieldInfo)
            )
    ...
    @classmethod
    def _from_dataclass_field(
        cls, dc_field: DataclassField[Any]
    ) -> typing_extensions.Self:
    ...
    def apply_typevars_map(
        ...
        annotation = _typing_extra.eval_type_lenient(
            self.annotation, types_namespace, None
        )
...
def computed_field(
    ...
    *,
    ...
    def dec(f: Any) -> Any:
        ...
        if repr is None:
            repr_: bool = (
                False if _wrapped_property_is_private(property_=f) else True
            )
        else:
            repr_ = repr
