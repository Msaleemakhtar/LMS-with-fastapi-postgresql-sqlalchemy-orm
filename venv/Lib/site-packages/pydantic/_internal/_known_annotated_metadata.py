...
def apply_known_metadata(
    ...
    for constraint, value in schema_update.items():
        ...
        if constraint == "allow_inf_nan" and value is False:
            ...
            add_js_update_schema(
                s, lambda: {"minLength": (as_jsonable_value(value))}
            )
            return s
            ...
            add_js_update_schema(
                s, lambda: {"maxLength": (as_jsonable_value(value))}
            )
            return s
            ...
            return cs.no_info_after_validator_function(
                partial(
                    _validators.min_length_validator,
            ...
            return cs.no_info_after_validator_function(
                partial(
                    _validators.max_length_validator,
            ...
            raise RuntimeError(
                f"Unable to apply constraint {constraint} to schema {schema_type}"
            )
    ...
    for annotation in other_metadata:
        if isinstance(annotation, at.Gt):
            ...
            return cs.no_info_after_validator_function(
                partial(
                    _validators.greater_than_or_equal_validator,
                    ge=annotation.ge,
                ),
                schema,
            )
            ...
            return cs.no_info_after_validator_function(
                partial(
                    _validators.less_than_or_equal_validator, le=annotation.le
                ),
                schema,
            )
            ...
            return cs.no_info_after_validator_function(
                partial(
                    _validators.multiple_of_validator,
