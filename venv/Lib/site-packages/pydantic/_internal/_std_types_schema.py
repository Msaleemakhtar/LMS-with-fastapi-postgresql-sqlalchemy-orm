...
def get_enum_core_schema(
    enum_type: type[Enum], config: ConfigDict
) -> CoreSchema:
    ...
    if issubclass(enum_type, int):
        ...
        lax = core_schema.chain_schema(
            [core_schema.int_schema(), to_enum_validator]
        )
        ...
        updates["type"] = "string"
        lax = core_schema.chain_schema(
            [core_schema.str_schema(), to_enum_validator]
        )
        strict = core_schema.json_or_python_schema(
            json_schema=core_schema.no_info_after_validator_function(
        ...
        lax = core_schema.chain_schema(
            [core_schema.float_schema(), to_enum_validator]
        )
        strict = core_schema.json_or_python_schema(
            json_schema=core_schema.no_info_after_validator_function(
...
def decimal_prepare_pydantic_annotations(
    ...
    _known_annotated_metadata.check_metadata(
        metadata,
        {
            *_known_annotated_metadata.FLOAT_CONSTRAINTS,
            "max_digits",
            "decimal_places",
        },
        decimal.Decimal,
...
def path_schema_prepare_pydantic_annotations(
    ...
    construct_path = (
        pathlib.PurePath if source_type is os.PathLike else source_type
    )
    ...
    def __get_pydantic_core_schema__(
        ...
        if self.mapped_origin in (list, set, frozenset):
            if self.mapped_origin is list:
                ...
                constrained_schema = core_schema.set_schema(
                    items_schema, **metadata
                )
            else:
                assert (
            ...
            if self.mapped_origin is collections.deque:
                ...
                coerce_instance_wrap = partial(
                    ...
                    partial(
                        dequeue_validator,
                        maxlen=metadata.get("max_length", None),
                    ),
                )
            else:
                coerce_instance_wrap = partial(
...
def sequence_like_prepare_pydantic_annotations(
    ...
    if not args:
        ...
        raise ValueError(
            "Expected sequence to have exactly 1 generic parameter"
        )
