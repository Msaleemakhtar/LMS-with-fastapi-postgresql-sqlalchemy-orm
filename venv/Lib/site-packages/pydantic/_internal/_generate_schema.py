...
def check_decorator_fields_exist(
    ...
    for dec in decorators:
        if (
            isinstance(
                dec.info, (ValidatorDecoratorInfo, FieldValidatorDecoratorInfo)
            )
...
class GenerateSchema:
    ...
    def _tuple_variable_schema(self, tp: Any, items_type: Any) -> CoreSchema:
        return core_schema.tuple_variable_schema(
            self.generate_schema(items_type)
        )

    def _tuple_positional_schema(
        self, tp: Any, items_types: list[Any]
    ) -> CoreSchema:
        items_schemas = [
            self.generate_schema(items_type) for items_type in items_types
        ]
    ...
    def _unpack_refs_defs(self, schema: CoreSchema) -> CoreSchema:
        ...
        if schema["type"] == "definitions":
            self.defs.definitions.update(
                {get_ref(s): s for s in schema["definitions"]}
            )
    ...
    def _generate_schema_from_property(
        ...
        ref = get_ref(schema)
        if ref:
            self.defs.definitions[ref] = self._post_process_generated_schema(
                schema
            )
    ...
    def _resolve_forward_ref(self, obj: Any) -> Any:
        ...
        try:
            obj = _typing_extra.evaluate_fwd_ref(
    ...
    def _generate_schema(self, obj: Any) -> core_schema.CoreSchema:
        ...
        self._needs_apply_discriminated_union = False
        schema = self._post_process_generated_schema(
            self._generate_schema_inner(obj)
        )
        self._has_invalid_schema = (
            self._has_invalid_schema or has_invalid_schema
        )
        self._needs_apply_discriminated_union = (
            self._needs_apply_discriminated_union
    ...
    def match_type(self, obj: Any) -> core_schema.CoreSchema:  # noqa: C901
        ...
        if obj is str:
            ...
            return self._dict_schema(
                obj, *self._get_first_two_args_or_any(obj)
            )
    ...
    def _dataclass_schema(
        ...
        with self.defs.get_schema_or_ref(dataclass) as (
            dataclass_ref,
            maybe_schema,
        ):
            if maybe_schema is not None:
    ...
    def _unsubstituted_typevar_schema(
        ...
        if (bound is not None) + (len(constraints) != 0) + (
            default is not None
        ) > 1:
