...
RunnerType = Callable[
    [Sequence[str], Optional[str], Optional[Mapping[str, str]]], None
]
...
def _parse_build_system_table(
    pyproject_toml: Mapping[str, Any]
) -> Mapping[str, Any]:
    ...
    if "backend-path" in build_system_table and (
        ...
        or not all(
            isinstance(i, str) for i in build_system_table["backend-path"]
        )
    ):
        msg = "`backend-path` must be an array of strings"
        raise BuildSystemTableValidationError(msg)

    unknown_props = build_system_table.keys() - {
        "requires",
        "build-backend",
        "backend-path",
    }
...
def _wrap_subprocess_runner(
    runner: RunnerType, env: env.IsolatedEnv
) -> RunnerType:
    def _invoke_wrapped_runner(
        cmd: Sequence[str],
        cwd: str | None,
        extra_environ: Mapping[str, str] | None,
    ) -> None:
        runner(
            cmd,
            cwd,
            {**(env.make_extra_environ() or {}), **(extra_environ or {})},
        )
...
class ProjectBuilder:
    ...
    def __init__(
        ...
        self._build_system = _parse_build_system_table(
            _read_pyproject_toml(pyproject_toml_path)
        )
    ...
    def check_dependencies(
        ...
        """
        dependencies = self.get_requires_for_build(
            distribution, config_settings
        ).union(self.build_system_requires)
    ...
    def build(
        ...
        kwargs = (
            ...
            else {"metadata_directory": metadata_directory}
        )
        return self._call_backend(
            f"build_{distribution}",
            output_directory,
            config_settings,
            **kwargs,
        )
    ...
    def _handle_backend(self, hook: str) -> Iterator[None]:
        try:
            ...
            raise BuildBackendException(
                exception,
                f"Backend subprocess exited when trying to invoke {hook}",
            ) from None
