...
class DefaultIsolatedEnv(IsolatedEnv):
        ...
        try:
            ...
            if _should_use_virtualenv():
                ...
                (
                    ...
                    self._scripts_dir,
                ) = _create_isolated_env_virtualenv(self._path)
            else:
                ...
                self._path = os.path.realpath(
                    tempfile.mkdtemp(prefix="build-env-")
                )
                (
                    self._python_executable,
                    self._scripts_dir,
                ) = _create_isolated_env_venv(self._path)
    ...
    def make_extra_environ(self) -> dict[str, str]:
        ...
        return {
            "PATH": os.pathsep.join([self._scripts_dir, path])
            if path is not None
            else self._scripts_dir
        }
    ...
    def install(self, requirements: Collection[str]) -> None:
        ...
        self.log(
            f'Installing packages in isolated environment... ({", ".join(sorted(requirements))})'
        )
