...
class Container(_CustomDict):
    ...
    def add(
        self, key: Key | Item | str, item: Item | None = None
    ) -> Container:
        """
    ...
    def _handle_dotted_key(self, key: Key, value: Item) -> None:
        ...
        name._dotted = True
        table = current = Table(
            Container(True), Trivia(), False, is_super_table=True
        )
        for _name in mid:
            _name._dotted = True
            new_table = Table(
    ...
    def _render_table(
        self, key: Key, table: Table, prefix: str | None = None
    ) -> str:
        cur = ""
        ...
        if not table.is_super_table() or (
            ...
            newline_in_table_trivia = (
                "\n"
                if "\n" not in table.trivia.trail and len(table.value) > 0
                else ""
            )
            cur += (
    ...
    def _render_aot_table(
        self, table: Table, prefix: str | None = None
    ) -> str:
        cur = ""
        _key = prefix or ""
        open_, close = "[[", "]]"
    ...
    def _replace(
        self, key: Key | str, new_key: Key | str, value: Item
    ) -> None:
        if not isinstance(key, Key):
            key = SingleKey(key)
...
def ends_with_whitespace(it: Any) -> bool:
    ...
    return (
        isinstance(it, Table)
        and isinstance(it.value._previous_item(), Whitespace)
    ) or (
        isinstance(it, AoT) and len(it) > 0 and isinstance(it[-1], Whitespace)
    )
