...
class Parser:
    ...
    def parse(self) -> TOMLDocument:
        ...
        while not self.end():
            ...
            key, value = item
            if (key is not None and key.is_multi()) or not self._merge_ws(
                value, body
            ):
    ...
    def _parse_table(
        ...
        if any(
            " " in part.key.strip() and part.is_bare() for part in name_parts
        ):
            raise self.parse_error(
        ...
        if isinstance(result, Null):
            ...
            if is_aot and (
                not self._aot_stack or full_key != self._aot_stack[-1]
            ):
