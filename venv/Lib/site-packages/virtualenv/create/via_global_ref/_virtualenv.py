...
def patch_dist(dist):
    ...
    def parse_config_files(self, *args, **kwargs):
        ...
        if (
            ...
            install["prefix"] = VIRTUALENV_PATCH_FILE, os.path.abspath(
                sys.prefix
            )
            ...
            with self.lock[0]:
                ...
                try:
                    ...
                    if spec is not None:
                        ...
                        func_name = (
                            "exec_module" if is_new_api else "load_module"
                        )
                        old = getattr(spec.loader, func_name)
                        func = (
                            self.exec_module
                            if is_new_api
                            else self.load_module
                        )
                        if old is not func:
                            with suppress(
                                AttributeError
                                ...
                                setattr(
                                    spec.loader, func_name, partial(func, old)
                                )
                        return spec
                finally:
                    self.fullname = None
