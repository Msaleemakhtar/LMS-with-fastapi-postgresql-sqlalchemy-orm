...
class PythonInfo:
    ...
    def __init__(self) -> None:  # noqa: PLR0915
        ...
        self.real_prefix = abs_path(
            getattr(sys, "real_prefix", None)
        )  # old virtualenv

        # information about the exec prefix - dynamic stdlib modules
        self.base_exec_prefix = abs_path(
        ...
        if "venv" in scheme_names:
            ...
            self.sysconfig_paths = {
                i: sysconfig.get_path(
                    i, expand=False, scheme=self.sysconfig_scheme
                )
    ...
    def install_path(self, key):
        ...
        if (
            ...
            config_var = {
                k: "" if v in prefixes else v
                for k, v in self.sysconfig_vars.items()
            }
            result = self.sysconfig_path(key, config_var=config_var).lstrip(
                os.sep
            )
        return result
    ...
    def satisfies(self, spec, impl_must_match):  # noqa: C901
        ...
        if spec.path:
            if self.executable == os.path.abspath(spec.path):
                return (
                    True  # if the path is a our own executable path we're done
                )
            if not spec.is_abs:
        ...
        if (
            ...
            and spec.architecture != self.architecture
        ):
            return False
        ...
        while target.system_executable is None:
            ...
            if prefix in prefixes:
                if len(prefixes) == 1:
                    ...
                    logging.info(
                        "%r links back to itself via prefixes", target
                    )
    ...
    def discover_exe(
        self, app_data, prefix, exact=True, env=None
    ):  # noqa: FBT002
        key = prefix, exact
        ...
        for folder in possible_folders:
            for name in possible_names:
                info = self._check_exe(
                    app_data, folder, name, exact, discovered, env
                )
                if info is not None:
    ...
    def _select_most_likely(discovered, target):
        ...
        def sort_by(info):
            ...
            matches = [
                ...
                info.version_info.releaselevel
                == target.version_info.releaselevel,
    ...
    def _possible_base(self):
        ...
        basename = os.path.splitext(os.path.basename(self.executable))[
            0
        ].rstrip(digits)
