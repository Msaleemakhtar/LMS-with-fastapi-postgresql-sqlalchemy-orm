...
class Builtin(Discover):
    def __init__(self, options) -> None:
        ...
        self.python_spec = (
            options.python if options.python else [sys.executable]
        )
    ...
    def __repr__(self) -> str:
        spec = (
            self.python_spec[0]
            if len(self.python_spec) == 1
            else self.python_spec
        )
    ...
    for py_exe in try_first_with:
        ...
        try:
            ...
            yield PythonInfo.from_exe(
                os.path.abspath(path), app_data, env=env
            ), True
...
def get_paths(env):
    ...
    return (
        []
        if not path
        else [p for p in path.split(os.pathsep) if os.path.exists(p)]
    )
        ...
        if self.env.get(
            ...
            for file_name in os.listdir(self.path):
                try:
                    ...
                    if os.path.isdir(file_path) or not os.access(
                        file_path, os.X_OK
                    ):
                        continue
                except OSError:
