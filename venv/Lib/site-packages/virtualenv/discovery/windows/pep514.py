...
def process_set(hive, hive_name, key, flags, default_arch):
    try:
        with winreg.OpenKeyEx(
            hive, key, 0, winreg.KEY_READ | flags
        ) as root_key:
            for company in enum_keys(root_key):
                if company == "PyLauncher":  # reserved
                    continue
                yield from process_company(
                    hive_name, company, root_key, default_arch
                )
    except OSError:
        pass
...
def process_company(hive_name, company, root_key, default_arch):
    with winreg.OpenKeyEx(root_key, company) as company_key:
        for tag in enum_keys(company_key):
            spec = process_tag(
                hive_name, company, company_key, tag, default_arch
            )
            if spec is not None:
                yield spec
...
def process_tag(hive_name, company, company_key, tag, default_arch):
    with winreg.OpenKeyEx(company_key, tag) as tag_key:
        ...
        if version is not None:  # if failed to get version bail
            ...
            arch = load_arch_data(
                hive_name, company, tag, tag_key, default_arch
            )
            if arch is not None:
...
def load_version_data(hive_name, company, tag, tag_key):
    for candidate, key_path in [
        (
            get_value(tag_key, "SysVersion"),
            f"{hive_name}/{company}/{tag}/SysVersion",
        ),
...
def parse_version(version_str):
    if isinstance(version_str, str):
        ...
        if match:
            return tuple(
                int(i) if i is not None else None for i in match.groups()
            )
...
def msg(path, what):
    LOGGER.warning(
        "PEP-514 violation in Windows Registry at %s error: %s", path, what
    )
