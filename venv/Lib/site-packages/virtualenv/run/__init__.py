...
def session_via_cli(
    args, options=None, setup_logging=True, env=None
):  # noqa: FBT002
    """
...
def build_parser(
    args=None, options=None, setup_logging=True, env=None
):  # noqa: FBT002
    parser = VirtualEnvConfigParser(
        options, os.environ if env is None else env
    )
...
def load_app_data(args, parser, options):
    ...
    parser.add_argument(
        ...
        type=partial(
            make_app_data,
            read_only=options.read_only_app_data,
            env=options.env,
        ),
...
def _do_report_setup(parser, args, setup_logging):
    level_map = ", ".join(
        f"{logging.getLevelName(line)}={c}"
        for c, line in sorted(LEVELS.items())
    )
