...
def from_bundle(
    ...
    wheel = load_embed_wheel(
        app_data, distribution, for_py_version, of_version
    )
    ...
    if version != Version.embed:
        ...
        found_wheel = from_dir(
            distribution, of_version, for_py_version, search_dirs
        )
...
def from_dir(distribution, version, for_py_version, directories):
    ...
    for folder in directories:
        for wheel in discover_wheels(
            folder, distribution, version, for_py_version
        ):
            return wheel
    return None
