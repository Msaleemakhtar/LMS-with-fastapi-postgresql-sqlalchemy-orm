...
def download_wheel(
    ...
    logging.debug(
        "download wheel %s %s to %s", to_download, for_py_version, to_folder
    )
    cmd = [
        sys.executable,
        "-m",
...
def find_compatible_in_house(
    distribution, version_spec, for_py_version, in_folder
):
    ...
    if version_spec is not None and version_spec:
        ...
        start = next(
            (
                at
                for at, w in enumerate(wheels)
                if op(w.version_tuple, version)
            ),
            len(wheels),
        )
