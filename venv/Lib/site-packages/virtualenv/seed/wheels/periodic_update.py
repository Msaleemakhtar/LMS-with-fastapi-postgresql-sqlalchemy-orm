    ...
    if of_version is None:
        for _, group in groupby(
            ...
            for version in all_patches:
                if (
                    wheel is not None
                    and Path(version.filename).name == wheel.name
                ):
                    return wheel
...
def add_wheel_to_update_log(wheel, for_py_version, app_data):
    embed_update_log = app_data.embed_update_log(
        wheel.distribution, for_py_version
    )
    logging.debug(
        "adding %s information to %s", wheel.name, embed_update_log.file
    )
    u_log = UpdateLog.from_dict(embed_update_log.read())
    if any(version.filename == wheel.name for version in u_log.versions):
        logging.warning(
            "%s already present in %s", wheel.name, embed_update_log.file
        )
        return
    # we don't need a release date for sources other than "periodic"
    version = NewVersion(
        wheel.name, datetime.now(tz=timezone.utc), None, "download"
    )
    u_log.versions.append(
        version
...
def load_datetime(value):
    return (
        None
        if value is None
        else datetime.strptime(value, DATETIME_FMT).replace(
            tzinfo=timezone.utc
        )
    )
...
class NewVersion:
    ...
    def use(
        self,
        now,
        ignore_grace_period_minor=False,
        ignore_grace_period_ci=False,
    ):  # noqa: FBT002
...
class UpdateLog:
    ...
    def from_app_data(cls, app_data, distribution, for_py_version):
        raw_json = app_data.embed_update_log(
            distribution, for_py_version
        ).read()
    ...
    def _check_start(self, now):
        return (
            self.started is None or now - self.started > UPDATE_ABORTED_DELAY
        )
...
def do_update(
    ...
    try:
        ...
        logging.debug(
            "done %s %s with %s", distribution, for_py_version, versions
        )
    return versions
...
def _run_do_update(  # noqa: C901, PLR0913
    ...
    app_data = (
        AppDataDiskFolder(app_data) if isinstance(app_data, str) else app_data
    )
    ...
    while last is None or not last.use(now, ignore_grace_period_ci=True):
        ...
        logging.info(
            "detected %s in %s",
            last,
            datetime.now(tz=timezone.utc) - download_time,
        )
        ...
        if (
            embed_version is not None
            and embed_version >= last_wheel.version_tuple
        ):
