...
class PipInstall(metaclass=ABCMeta):
        ...
        for name, module in self._console_scripts.items():
            consoles.update(
                self._create_console_entry_point(
                    name, module, script_dir, version_info
                )
            )
        logging.debug(
    ...
    def build_image(self):
        ...
        logging.debug(
            "build install image for %s to %s",
            self._wheel.name,
            self._image_dir,
        )
    ...
    def _records_text(self, files):
        return "\n".join(
            f"{os.path.relpath(str(rec), str(self._image_dir))},,"
            for rec in files
        )
    ...
    def _create_console_entry_point(
        self, name, value, to_folder, version_info
    ):
        result = []
        maker = ScriptMakerCustom(
            to_folder, version_info, self._creator.exe, name
        )
    ...
    def _uninstall_dist(dist):
        ...
        for name in (
            (
                i.split(",")[0]
                for i in record.read_text(encoding="utf-8").splitlines()
            )
            if record.exists()
            else ()
        ):
    ...
    def has_image(self):
        return (
            ...
            and next(self._image_dir.iterdir()) is not None
        )
