...
class FromAppData(BaseEmbed):
    ...
    def run(self, creator):
        ...
        with self._get_seed_wheels(creator) as name_to_whl:
            pip_version = (
                name_to_whl["pip"].version_tuple
                if "pip" in name_to_whl
                else None
            )
            ...
            threads = [
                Thread(target=_install, args=(n, w))
                for n, w in name_to_whl.items()
            ]
    ...
    def installer_class(self, pip_version_tuple):
        if (
            self.symlinks
            and pip_version_tuple
            and pip_version_tuple >= (19, 3)
