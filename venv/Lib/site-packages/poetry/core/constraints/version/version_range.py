...
class VersionRange(VersionRangeConstraint):
    ...
    def allows(self, other: Version) -> bool:
        if self._min is not None:
            ...
            if not self._include_min and (
                _other == self._min or _other == _this
            ):
                return False
        ...
        if self.max is not None:
            ...
            if not self._include_max and (
                _other == self._max or _other == _this
            ):
                return False
    ...
    def allows_all(self, other: VersionConstraint) -> bool:
        ...
        if isinstance(other, VersionUnion):
            return all(
                self.allows_all(constraint) for constraint in other.ranges
            )

        if isinstance(other, VersionRangeConstraint):
            return not other.allows_lower(self) and not other.allows_higher(
                self
            )
    ...
    def allows_any(self, other: VersionConstraint) -> bool:
        ...
        if isinstance(other, VersionUnion):
            return any(
                self.allows_any(constraint) for constraint in other.ranges
            )

        if isinstance(other, VersionRangeConstraint):
            return not (
                other.is_strictly_lower(self) or other.is_strictly_higher(self)
            )
    ...
    def union(self, other: VersionConstraint) -> VersionConstraint:
        ...
        if isinstance(other, VersionRangeConstraint):
            ...
            edges_touch = (
                self.max == other.min
                and (self.include_max or other.include_min)
            ) or (
                self.min == other.max
                and (self.include_min or other.include_max)
            )
    ...
    def difference(self, other: VersionConstraint) -> VersionConstraint:
        ...
        if isinstance(other, Version):
            ...
            if other == self.min:
                ...
                return VersionRange(
                    self.min, self.max, False, self.include_max
                )
            ...
            if other == self.max:
                ...
                return VersionRange(
                    self.min, self.max, self.include_min, False
                )
