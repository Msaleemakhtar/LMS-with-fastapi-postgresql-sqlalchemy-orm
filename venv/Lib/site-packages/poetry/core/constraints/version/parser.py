...
def parse_marker_version_constraint(constraints: str) -> VersionConstraint:
    return _parse_constraint(
        constraints=constraints, is_marker_constraint=True
    )
...
def parse_single_constraint(
    ...
    from poetry.core.constraints.version.patterns import (
    ...
    if m:
        ...
        if op == "!=":
            return VersionUnion(
                VersionRange(max=version), VersionRange(min=version)
            )
...
def _make_x_constraint_range(
    ...
    *,
    invert: bool = False,
    is_marker_constraint: bool = False,
) -> VersionConstraint:
    from poetry.core.constraints.version.version_range import VersionRange
    ...
    if version.is_postrelease():
        ...
        raise RuntimeError(
            "version is neither stable, nor pre-release nor dev-release"
        )
