...
def make_recons_rule(origin, expansion, old_expansion):
    return Rule(
        origin, expansion, alias=_MakeTreeMatch(origin.name, old_expansion)
    )
...
class TreeMatcher:
    ...
    def __init__(self, parser):
        ...
        self.tokens, rules, _extra = parser.grammar.compile(
    ...
    def match_tree(self, tree, rulename):
        ...
        try:
            ...
            rules = self.rules + _best_rules_from_group(
                self.rules_for_root[rulename]
            )
            ...
            parser = earley.Parser(
                self.parser.lexer_conf, conf, _match, resolve_ambiguity=True
            )
