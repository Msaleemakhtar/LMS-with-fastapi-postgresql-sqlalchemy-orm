...
class TemplateConf:
    ...
    def _match_tree_template(
        ...
        if template.data == tree.data and len(template.children) == len(
            tree.children
        ):
            res = {}
...
class _ReplaceVars(Transformer[str, Tree[str]]):
    def __init__(
        self, conf: TemplateConf, vars: Mapping[str, Tree[str]]
    ) -> None:
        super().__init__()
        self._conf = conf
        self._vars = vars
        ...
        if var:
            try:
                ...
                raise MissingVariableError(
                    f"No mapping for template variable ({var})"
                )
        return tree
...
class Template:
    ...
    def search(
        self, tree: TreeOrCode
    ) -> Iterator[Tuple[Tree[str], MatchResult]]:
...
def _get_template_name(value: str) -> Optional[str]:
    return (
        ...
        if value.startswith(_TEMPLATE_MARKER)
        else None
    )
