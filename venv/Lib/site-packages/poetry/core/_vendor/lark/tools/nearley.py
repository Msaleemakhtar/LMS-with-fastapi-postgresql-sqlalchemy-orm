    ...
    for statement in tree.children:
        if statement.data == "directive":
            ...
            if directive in ("builtin", "include"):
                folder = (
                    builtin_path if directive == "builtin" else folder_path
                )
...
def create_code_for_nearley_grammar(
    g, start, builtin_path, folder_path, es6=False
):
    import js2py
    ...
    n2l = NearleyToLark()
    rule_defs = _nearley_to_lark(
        g, builtin_path, n2l, js_code, folder_path, set()
    )
    lark_g = "\n".join(rule_defs)
    lark_g += "\n" + "\n".join(
        "!%s: %s" % item for item in n2l.extra_rules.items()
    )
    ...
    emit()
    emit(
        'parser = Lark(grammar, start="n_%s", maybe_placeholders=False)'
        % start
    )
    emit("def parse(text):")
...
def main(fn, start, nearley_lib, es6=False):
    with codecs.open(fn, encoding="utf8") as f:
        grammar = f.read()
    return create_code_for_nearley_grammar(
        grammar,
        start,
        os.path.join(nearley_lib, "builtin"),
...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        description="Reads a Nearley grammar (with js functions), and outputs an equivalent lark parser."
    )
    parser.add_argument(
        "nearley_grammar",
        help="Path to the file containing the nearley grammar",
    )
    parser.add_argument(
        "start_rule",
        help="Rule within the nearley grammar to make the base rule",
    )
    parser.add_argument(
        "nearley_lib",
        help="Path to root directory of nearley codebase (used for including builtins)",
    )
    parser.add_argument(
        "--es6", help="Enable experimental ES6 support", action="store_true"
    )
    return parser
...
if __name__ == "__main__":
    ...
    print(
        main(
            fn=args.nearley_grammar,
            start=args.start_rule,
            nearley_lib=args.nearley_lib,
            es6=args.es6,
        )
    )
