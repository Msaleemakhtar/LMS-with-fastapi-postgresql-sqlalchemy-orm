...
def _create_unless(terminals, g_regex_flags, re_, use_bytes):
    ...
    for retok in tokens_by_type.get(PatternRE, []):
        ...
        for strtok in tokens_by_type.get(PatternStr, []):
            ...
            if s == _get_match(
                re_, retok.pattern.to_regexp(), s, g_regex_flags
            ):
...
def _check_regex_collisions(
    ...
    for group in classify(terminal_to_regexp, lambda t: t.priority).values():
        for a, b in comparator.check(group, skip_marked=True):
            ...
            if comparator.count_marked_pairs() >= max_collisions_to_show:
                logger.warning(
                    "Found 8 regex collisions, will not check for more."
                )
                return
        ...
        if not conf.skip_validation:
            ...
            if has_interegular:
                _check_regex_collisions(
                    terminal_to_regexp, comparator, conf.strict
                )
            elif conf.strict:
                raise LexError(
        ...
        while line_ctr.char_pos < len(lex_state.text):
            ...
            if not res:
                ...
                raise UnexpectedCharacters(
                    ...
                    line_ctr.column,
                    allowed=allowed,
                    token_history=lex_state.last_token
                    and [lex_state.last_token],
                    state=parser_state,
...
class ContextualLexer(Lexer):
    ...
    def __init__(
        ...
        trad_conf.skip_validation = (
            True  # We don't need to verify all terminals again
        )
