...
class Item:
    ...
    def __init__(self, rule, ptr, start):
        ...
        if self.is_complete:
            ...
            self.expect = None
            self.previous = (
                rule.expansion[ptr - 1]
                if ptr > 0 and len(rule.expansion)
                else None
            )
        else:
            self.s = (rule, ptr)
    ...
    def __eq__(self, other):
        return self is other or (
            self.s == other.s and self.start == other.start
        )
    ...
    def __repr__(self):
        before = (
            expansion.name for expansion in self.rule.expansion[: self.ptr]
        )
        after = (
            expansion.name for expansion in self.rule.expansion[self.ptr :]
        )
        symbol = "{} ::= {}* {}".format(
            self.rule.origin.name, " ".join(before), " ".join(after)
        )
