...
class Parser:
    ...
    def __init__(
        ...
        for rule in parser_conf.rules:
            ...
            if (
                self.forest_sum_visitor is None
                and rule.options.priority is not None
            ):
        ...
        if (
            self.lexer_conf.lexer_type != "basic"
            and self.forest_sum_visitor is None
        ):
            ...
            if item.is_complete:  ### (item.s == string)
                if item.node is None:
                    ...
                    item.node = (
                        ...
                        else node_cache.setdefault(
                            label, self.SymbolNode(*label)
                        )
                    )
                    item.node.add_family(
                        item.s, item.rule, item.start, None, None
                    )
                ...
                if item.rule.origin in transitives[item.start]:
                    ...
                    new_item = Item(
                        transitive.rule, transitive.ptr, transitive.start
                    )
                ...
                ###R Regular Earley completer
                    ...
                    originators = [
                        ...
                        if originator.expect is not None
                        and originator.expect == item.s
                    ]
        ...
        def is_quasi_complete(item):
            ...
            while not quasi.is_complete:
                if quasi.expect not in self.NULLABLE:
                    return False
                if (
                    quasi.rule.origin == start_symbol
                    and quasi.expect == start_symbol
                ):
                    return False
                quasi = quasi.advance()
            return True
    ...
    def parse(self, lexer, start):
        ...
        if not solutions:
            ...
            raise UnexpectedEOF(
                expected_terminals, state=frozenset(i.s for i in to_scan)
            )
