...
class Parser:
    ...
    def _to_rule(self, lark_rule):
        ...
        return Rule(
            lark_rule.origin,
            lark_rule.expansion,
            weight=lark_rule.options.priority
            if lark_rule.options.priority
            else 0,
            alias=lark_rule,
        )
...
class UnitSkipRule(Rule):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.skipped_rules == other.skipped_rules
        )
...
def _term(g):
    ...
    t_rules = {
        t: Rule(NT("__T_%s" % str(t)), [t], weight=0, alias="Term")
        for t in all_t
    }
    new_rules = []
    for rule in g.rules:
        if len(rule.rhs) > 1 and any(isinstance(x, T) for x in rule.rhs):
            new_rhs = [
                t_rules[x].lhs if isinstance(x, T) else x for x in rule.rhs
            ]
            new_rules.append(
...
def unroll_unit_skiprule(
    lhs, orig_rhs, skipped_rules, children, weight, alias
):
    if not skipped_rules:
        return RuleNode(
            Rule(lhs, orig_rhs, weight=weight, alias=alias),
            children,
            weight=weight,
        )
    else:
...
def revert_cnf(node):
    ...
    if node.rule.lhs.name.startswith("__T_"):
        ...
        for child in map(revert_cnf, node.children):
            # Reverts BIN rule.
            if isinstance(child, RuleNode) and child.rule.lhs.name.startswith(
                "__SP_"
            ):
