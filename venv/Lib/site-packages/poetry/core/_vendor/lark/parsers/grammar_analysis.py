...
class GrammarAnalyzer:
    def __init__(
        ...
        self.rules_by_origin: Dict[NonTerminal, List[Rule]] = classify(
            rules, lambda r: r.origin
        )

        if len(rules) != len(set(rules)):
            duplicates = [
                item for item, count in Counter(rules).items() if count > 1
            ]
            raise GrammarError(
    ...
    def expand_rule(
        self, source_rule: NonTerminal, rules_by_origin=None
    ) -> State:
