...
class SymbolNode(ForestNode):
    ...
    def load_paths(self):
        for transitive, node in self.paths:
            if transitive.next_titem is not None:
                vn = type(self)(
                    transitive.next_titem.s,
                    transitive.next_titem.start,
                    self.end,
                )
    ...
    def __repr__(self):
        if self.is_intermediate:
            ...
            symbol = self.s.name
        return "({}, {}, {}, {})".format(
            symbol, self.start, self.end, self.priority
        )
...
class ForestToParseTree(ForestTransformer):
    ...
    def _check_cycle(self, node):
        if self._on_cycle_retreat:
            if (
                ...
                or id(node) in self._successful_visits
            ):
                self._cycle_node = None
                self._on_cycle_retreat = False
            else:
    ...
    def transform_packed_node(self, node, data):
        ...
        if (
            ...
            and id(node.parent) in self._successful_visits
        ):
            return Discard
        ...
        return self._cache.setdefault(
            id(node), self._call_rule_func(node, children)
        )
...
class TreeForestTransformer(ForestToParseTree):
    ...
    def _call_rule_func(self, node, data):
        ...
        if user_func == self.__default__ or hasattr(
    ...
    def visit_packed_node_out(self, node):
        ...
        for child in [node.left, node.right]:
            if child is not None:
                ...
                self.graph.add_edge(
                    self.pydot.Edge(graph_node, child_graph_node)
                )
            else:
