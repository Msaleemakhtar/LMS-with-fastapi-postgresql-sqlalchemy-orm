...
class CodeGeneratorDraft06(CodeGeneratorDraft04):
    FORMAT_REGEXS = dict(
        CodeGeneratorDraft04.FORMAT_REGEXS,
        **{
            "json-pointer": r"^(/(([^/~])|(~[01]))*)*\Z",
    ...
    def generate_exclusive_minimum(self):
        with self.l("if isinstance({variable}, (int, float, Decimal)):"):
            if not isinstance(
                self._definition["exclusiveMinimum"],
                (int, float, decimal.Decimal),
            ):
                raise JsonSchemaDefinitionException(
                    "exclusiveMinimum must be an integer, a float or a decimal"
                )
            with self.l("if {variable} <= {exclusiveMinimum}:"):
                self.exc(
                    "{name} must be bigger than {exclusiveMinimum}",
    ...
    def generate_exclusive_maximum(self):
        with self.l("if isinstance({variable}, (int, float, Decimal)):"):
            if not isinstance(
                self._definition["exclusiveMaximum"],
                (int, float, decimal.Decimal),
            ):
                raise JsonSchemaDefinitionException(
                    "exclusiveMaximum must be an integer, a float or a decimal"
                )
    ...
    def generate_const(self):
        ...
        with self.l("if {variable} != {}:", const):
            self.exc(
                "{name} must be same as const definition: {definition_rule}",
                rule="const",
            )
