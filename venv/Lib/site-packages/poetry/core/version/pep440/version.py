...
class PEP440Version:
    ...
    release: Release = dataclasses.field(
        default_factory=Release, compare=False
    )
    ...
    def _is_increment_required(self) -> bool:
        return self.is_stable() or (
            not self.is_prerelease() and self.is_postrelease()
        )
    ...
    def next_major(self: T) -> T:
        ...
        if (
            ...
            or Release(release.major, 0, 0) < release
        ):
    ...
    def next_stable(self: T) -> T:
        ...
        return self.__class__(
            epoch=self.epoch, release=release, local=self.local
        )
