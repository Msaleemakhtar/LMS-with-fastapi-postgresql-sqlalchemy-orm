...
class Factory:
    ...
    def _add_package_group_dependencies(
        ...
        if isinstance(group, str):
            if package.has_dependency_group(group):
                ...
                from poetry.core.packages.dependency_group import (
                    DependencyGroup,
                )
        ...
        if "readme" in config:
            if isinstance(config["readme"], str):
                ...
                package.readmes = tuple(
                    root / readme for readme in config["readme"]
                )
    ...
    def create_dependency(
        ...
        from poetry.core.packages.dependency import Dependency
        from poetry.core.packages.dependency_group import MAIN_GROUP
        from poetry.core.packages.directory_dependency import (
            DirectoryDependency,
        )
        ...
        if isinstance(constraint, dict):
            ...
            allows_prereleases = constraint.get(
                "allow-prereleases",
                constraint.get("allows-prereleases", False),
            )
                ...
                marker = marker.intersect(
                    parse_marker(
                        create_nested_marker(
                            "python_version",
                            parse_version_constraint(python_versions),
                        )
                    )
                )
    ...
    def validate(
        ...
        if strict:
            ...
            if "readme" in config and not isinstance(config["readme"], str):
                readme_types = {
                    readme_content_type(r) for r in config["readme"]
                }
