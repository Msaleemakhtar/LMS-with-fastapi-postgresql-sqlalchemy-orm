...
class SdistBuilder(Builder):
    ...
    def build(
        ...
        try:
            ...
            for file in sorted(
                files_to_add, key=lambda x: x.relative_to_source_root()
            ):
                tar_info = tar.gettarinfo(
                    str(file.path),
                    arcname=pjoin(
                        tar_dir, str(file.relative_to_source_root())
                    ),
                )
            ...
            if isinstance(include, PackageInclude):
                if include.is_package():
                    ...
                    if include.source is not None:
                        package_dir[""] = str(
                            include.base.relative_to(self._path)
                        )
            ...
            if is_subpkg:
                ...
                if len(data) == len(data_elements):
                    ...
                    for d in data:
                        ...
                        pkg_data[pkg] += [
                            pjoin(from_nearest_pkg, d.name) for d in data
                        ]
