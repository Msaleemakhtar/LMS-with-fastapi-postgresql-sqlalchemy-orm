...
def convert_markers(marker: BaseMarker) -> ConvertedMarkers:
    ...
    marker = dnf(marker)
    conjunctions = (
        marker.markers if isinstance(marker, MarkerUnion) else [marker]
    )
    ...
    for i, sub_marker in enumerate(conjunctions):
        if isinstance(sub_marker, MultiMarker):
            ...
            if isinstance(sub_marker, SingleMarker):
                ...
                add_constraint(
                    sub_marker.name, ("", str(sub_marker.constraint)), i
                )
...
def contains_group_without_marker(
    markers: ConvertedMarkers, marker_name: str
) -> bool:
...
def create_nested_marker(
    ...
    if isinstance(constraint, (MultiConstraint, UnionConstraint)):
        ...
        if isinstance(constraint, UnionConstraint):
            parts = [
                f"({part[1]})" if part[0] else part[1] for part in multi_parts
            ]
            glue = " or "
        else:
