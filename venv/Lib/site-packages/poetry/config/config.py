...
class PackageFilterPolicy:
    ...
    def normalize(cls, policy: str) -> list[str]:
        ...
        return list(
            {
                name.strip()
                if cls.is_reserved(name)
                else canonicalize_name(name)
...
class Config:
    ...
    def all(self) -> dict[str, Any]:
        def _all(
            config: dict[str, Any], parent_key: str = ""
        ) -> dict[str, Any]:
            all_ = {}
    ...
    def repository_cache_directory(self) -> Path:
        return (
            Path(self.get("cache-dir")).expanduser() / "cache" / "repositories"
        )
    ...
    def get(self, setting_name: str, default: Any = None) -> Any:
        ...
        if self._use_environment:
            ...
            env = "POETRY_" + "_".join(
                k.upper().replace("-", "_") for k in keys
            )
            env_value = os.getenv(env)
            if env_value is not None:
                return self.process(
                    self._get_normalizer(setting_name)(env_value)
                )
    ...
    def create(cls, reload: bool = False) -> Config:
        ...
        if _default_config is None or reload:
            ...
            if auth_config_file.exists():
                logger.debug(
                    "Loading configuration file %s", auth_config_file.path
                )
                _default_config.merge(auth_config_file.read())

            _default_config.set_auth_config_source(
                FileConfigSource(auth_config_file)
            )
