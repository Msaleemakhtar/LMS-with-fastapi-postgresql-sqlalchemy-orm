...
class Factory(BaseFactory):
    ...
    def create_poetry(
        ...
        locker = Locker(
            poetry_file.parent / "poetry.lock", base_poetry.local_config
        )
        ...
        plugin_manager = PluginManager(
            Plugin.group, disable_plugins=disable_plugins
        )
    ...
    def create_pool(
        ...
        for source in sources:
            ...
            priority = Priority[
                source.get("priority", Priority.PRIMARY.name).upper()
            ]
            ...
            if priority is Priority.SECONDARY:
                allowed_prios = (
                    p for p in Priority if p is not Priority.SECONDARY
                )
                warning = (
    ...
    def create_package_source(
        ...
        from poetry.repositories.pypi_repository import PyPiRepository
        from poetry.repositories.single_page_repository import (
        ...
        if name.lower() == "pypi":
            ...
            return PyPiRepository(
                disable_cache=disable_cache, pool_size=pool_size
            )
