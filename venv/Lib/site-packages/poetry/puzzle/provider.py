...
class Provider:
    ...
    def __init__(
        ...
        *,
        ...
        self._is_debugging: bool = (
            self._io.is_debug() or self._io.is_very_verbose()
        )
        ...
        self._locked: dict[
            NormalizedName, list[DependencyPackage]
        ] = defaultdict(list)
    ...
    def use_environment(self, env: Env) -> Iterator[Provider]:
        ...
        self._env = env
        self._python_constraint = Version.parse(
    ...
    @contextmanager
    def use_latest_for(
        self, names: Collection[NormalizedName]
    ) -> Iterator[Provider]:
    ...
    def search_for_direct_origin_dependency(
        ...
        if dependency.is_vcs():
            ...
            dependency._source_resolved_reference = (
    ...
    def search_for(self, dependency: Dependency) -> list[DependencyPackage]:
        ...
        direct_origin_package = self._direct_origin_packages.get(
            dependency.name
        )
        if direct_origin_package and direct_origin_package.satisfies(
            dependency
        ):
    ...
    def _search_for_vcs(self, dependency: VCSDependency) -> Package:
        ...
        self.validate_package_for_dependency(
            dependency=dependency, package=package
        )
    ...
    def _search_for_file(self, dependency: FileDependency) -> Package:
        ...
        package = self._direct_origin.get_package_from_file(
    ...
    def _search_for_directory(
        self, dependency: DirectoryDependency
    ) -> Package:
        dependency.validate(raise_error=True)
        package = self._direct_origin.get_package_from_directory(
            dependency.full_path
        )
    ...
    def complete_package(
        ...
        if self._load_deferred:
            ...
            for dep in _dependencies:
                if dep.is_direct_origin():
                    ...
                    if (
                        locked is not None
                        and locked.package.is_same_package_as(dep)
                    ):
                        continue
        ...
        for dep_name, deps in duplicates.items():
            ...
            deps = self._resolve_overlapping_markers(
                package, deps, active_extras
            )

            if len(deps) == 1:
                self.debug(
                    f"<debug>Merging requirements for {dep_name}</debug>"
                )
                dependencies.append(deps[0])
                continue
            ...
            for dep in deps:
                if not overrides_marker_intersection.intersect(
                    ...
                    current_overrides.update(
                        {dependency_package: package_overrides}
                    )
    ...
    def _merge_dependencies_by_constraint(
        ...
        for group in dep_groups:
            by_constraint: dict[
                VersionConstraint, list[Dependency]
            ] = defaultdict(list)
            for dep in group:
    ...
    def _is_relevant_marker(
        ...
        return (
            ...
            and (
                active_extras is None
                or marker.validate({"extra": active_extras})
            )
    ...
    def _resolve_overlapping_markers(
        ...
        for uses in itertools.product([True, False], repeat=len(dependencies)):
            ...
            for m in markers:
                used_marker_intersection = used_marker_intersection.intersect(
                    m
                )
            if not self._is_relevant_marker(
                used_marker_intersection, active_extras
            ):
                continue
            ...
            for dep in used_dependencies:
                if dep.is_direct_origin() or dep.source_name:
                    ...
                    if specific_source_dependency and (
                        ...
                        or dep.source_name
                        != specific_source_dependency.source_name
                    ):
