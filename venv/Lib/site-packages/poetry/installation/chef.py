...
class ChefInstallError(ChefError):
    def __init__(
        self, requirements: Collection[str], output: str, error: str
    ) -> None:
        message = "\n\n".join(
            (
...
class IsolatedEnv(BaseIsolatedEnv):
    ...
    def install(self, requirements: Collection[str]) -> None:
        ...
        package.python_versions = ".".join(
            str(v) for v in self._env.version_info[:3]
        )
...
class Chef:
        ...
        *,
        ...
        if archive.is_dir():
            destination = output_dir or Path(
                tempfile.mkdtemp(prefix="poetry-chef-")
            )
            return self._prepare(
                archive, destination=destination, editable=editable
            )
    ...
    def _prepare_sdist(
        self, archive: Path, destination: Path | None = None
    ) -> Path:
        ...
        with temporary_directory() as tmp_dir:
            ...
            if destination is None:
                destination = (
                    self._artifact_cache.get_cache_directory_for_link(
                        Link(archive.as_uri())
                    )
                )
