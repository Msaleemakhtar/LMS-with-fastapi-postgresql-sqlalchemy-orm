...
class Chooser:
    ...
    def choose_for(self, package: Package) -> Link:
        ...
        for link in self._get_links(package):
            if link.is_wheel:
                ...
                if not Wheel(link.filename).is_supported_by_environment(
                    self._env
                ):
                    logger.debug(
            ...
            if link.ext in {".egg", ".exe", ".msi", ".rpm", ".srpm"}:
                logger.debug(
                    "Skipping unsupported distribution %s", link.filename
                )
                continue
    ...
    def _sort_key(
        ...
        if link.is_wheel:
            ...
            pri = -(
                wheel.get_minimum_supported_index(self._env.supported_tags)
                or 0
            )
            if wheel.build_tag is not None:
                ...
                if not match:
                    raise ValueError(
                        f"Unable to parse build tag: {wheel.build_tag}"
                    )
        ...
        has_allowed_hash = int(
            self._is_link_hash_allowed_for_package(link, package)
        )
