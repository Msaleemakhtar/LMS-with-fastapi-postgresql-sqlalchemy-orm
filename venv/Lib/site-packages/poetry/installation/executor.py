...
class Executor:
    ...
    def _write(self, operation: Operation, line: str) -> None:
        if (
            not self.supports_fancy_output()
            ...
            try:
                ...
                with self._lock:
                    ...
                    if isinstance(e, ChefBuildError):
                        ...
                        message = (
                            f"<error>Cannot install {pkg.pretty_name}.</error>"
                        )
    ...
    def _increment_operations_count(
        self, operation: Operation, executed: bool
    ) -> None:
        with self._lock:
            if executed:
    ...
    def get_operation_message(
        ...
        if isinstance(operation, Update):
            initial_version = (
                initial_pkg := operation.initial_package
            ).version
    ...
    def _execute_uninstall(self, operation: Uninstall) -> int:
        ...
        message = (
            f"  <fg=blue;options=bold>•</> {op_msg}: <info>Removing...</info>"
        )
        self._write(operation, message)
    ...
    def _install(self, operation: Install | Update) -> int:
        ...
        if (
            ...
            and not self._use_modern_installation
        ):
        ...
        if not self._use_modern_installation:
            return self.pip_install(
                archive, upgrade=operation.job_type == "update"
            )
    ...
    def _prepare_git_archive(self, operation: Install | Update) -> Path:
        ...
        message = f"  <fg=blue;options=bold>•</> {operation_message}: <info>Cloning...</info>"
        self._write(operation, message)
        ...
        if package_poetry is not None:
            ...
            if package.develop and not package_poetry.package.build_script:
                ...
                with builder.setup_py():
                    return self.pip_install(
                        req, upgrade=True, editable=package.develop
                    )
    ...
    def _download_link(self, operation: Install | Update, link: Link) -> Path:
        ...
        if archive.suffix != ".whl":
            ...
            archive = self._chef.prepare(
                archive, output_dir=original_archive.parent
            )
    ...
    def _populate_hashes_dict(self, archive: Path, package: Package) -> None:
        if package.files and archive.name in {
            f["file"] for f in package.files
        }:
    ...
    def _download_archive(
        ...
        operation_message = self.get_operation_message(operation)
        message = f"  <fg=blue;options=bold>•</> {operation_message}: <info>Downloading...</>"
        progress = None
        if self.supports_fancy_output():
