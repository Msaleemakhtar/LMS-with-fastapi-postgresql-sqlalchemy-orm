...
class DependencyCache:
    ...
    def __init__(self, provider: Provider) -> None:
        ...
        self._search_for_cached = functools.lru_cache(maxsize=128)(
            self._search_for
        )
        ...
        while changed:
            ...
            for incompatibility in reversed(self._incompatibilities[package]):
                ...
                if result is _conflict:
                    ...
                    changed.clear()
                    changed.add(
                        str(self._propagate_incompatibility(root_cause))
                    )
                    break
                ...
                for level in range(
                    ...
                    if level in self._contradicted_incompatibilities_by_level:
                        self._contradicted_incompatibilities.difference_update(
                            self._contradicted_incompatibilities_by_level.pop(
                                level
                            ),
                        )
    ...
    def _choose_package_version(self) -> str | None:
        ...
        if locked is None:
            ...
            if package is None:
                ...
                self._add_incompatibility(
                    Incompatibility(
                        [Term(dependency, True)], NoVersionsCause()
                    )
                )
