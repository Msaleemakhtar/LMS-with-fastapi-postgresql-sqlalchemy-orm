...
class _Writer:
    ...
    def _visit(
        ...
        conjunction = (
            "So," if conclusion or incompatibility == self._root else "And"
        )
        ...
        if isinstance(cause.conflict.cause, ConflictCause) and isinstance(
            ...
            if conflict_line is not None and other_line is not None:
                ...
                single_line_conflict = self._is_single_line(
                ...
                if single_line_other or single_line_conflict:
                    first = (
                        cause.conflict if single_line_other else cause.other
                    )
                    second = (
                        cause.other if single_line_other else cause.conflict
                    )
                    self._visit(first)
                    self._visit(second)
                    self._write(
    ...
    def _is_collapsible(self, incompatibility: Incompatibility) -> bool:
        ...
        if not isinstance(
            cause.conflict.cause, ConflictCause
        ) and not isinstance(cause.other.cause, ConflictCause):
            return False
