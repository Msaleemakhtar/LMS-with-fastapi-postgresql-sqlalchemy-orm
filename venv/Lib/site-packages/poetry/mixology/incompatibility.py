...
class Incompatibility:
    def __init__(self, terms: list[Term], cause: IncompatibilityCause) -> None:
        ...
        if (
            ...
            and any(
                term.is_positive() and term.dependency.is_root
                for term in terms
            )
        ):
            terms = [
        ...
        if len(terms) != 1 and (
            ...
            len(terms) != 2
            or terms[0].dependency.complete_name
            == terms[-1].dependency.complete_name
        ):
            ...
            for term in terms:
                ...
                if ref in by_ref:
                    ...
                    err_msg = (
                        f"Package '{ref}' is listed as a dependency of itself."
                    )
    ...
    def __str__(self) -> str:
        ...
        if len(self._terms) == 2:
            ...
            if term1.is_positive() == term2.is_positive():
                if not term1.is_positive():
                    return (
                        f"either {self._terse(term1)} or {self._terse(term2)}"
                    )
        ...
        if positive and negative:
            if len(positive) != 1:
                return (
                    f"if {' and '.join(positive)} then {' or '.join(negative)}"
                )

            positive_term = next(
                term for term in self._terms if term.is_positive()
            )
            return (
    ...
    def _try_requires_both(
        ...
        this_positive = self._single_term_where(
            lambda term: term.is_positive()
        )
        if this_positive is None:
            return None

        other_positive = other._single_term_where(
            lambda term: term.is_positive()
        )
    ...
    def _try_requires_through(
        ...
        this_negative = self._single_term_where(
            lambda term: not term.is_positive()
        )
        other_negative = other._single_term_where(
            lambda term: not term.is_positive()
        )
        ...
        other_positive = self._single_term_where(
            lambda term: term.is_positive()
        )
    ...
    def _try_requires_forbidden(
        ...
        negative = prior._single_term_where(
            lambda term: not term.is_positive()
        )
        if negative is None:
            return None
