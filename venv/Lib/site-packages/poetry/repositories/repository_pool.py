...
class RepositoryPool(AbstractRepository):
    ...
    def all_repositories(self) -> list[Repository]:
        return [
            prio_repo.repository for prio_repo in self._sorted_repositories
        ]
    ...
    def _sorted_repositories(self) -> list[PrioritizedRepository]:
        return sorted(
            ...
            key=lambda prio_repo: prio_repo.priority,
        )
    ...
    def _contains_priority(self, priority: Priority) -> bool:
        return any(
            prio_repo.priority is priority
            for prio_repo in self._repositories.values()
        )
    ...
    def find_packages(self, dependency: Dependency) -> list[Package]:
        ...
        for repo in self.repositories:
            if (
                packages
                and self.get_priority(repo.name) is Priority.SUPPLEMENTAL
            ):
                break
