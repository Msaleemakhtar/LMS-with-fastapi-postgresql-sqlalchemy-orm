    ...
    def _normalise(self, remote_refs: FetchPackResult) -> None:
        ...
        if self.revision:
            ...
            and (
                ...
                or annotated_tag(f"refs/tags/{self.branch}")
                in remote_refs.refs
            )
        ):
...
class Git:
    ...
    def _clone(cls, url: str, refspec: GitRefSpec, target: Path) -> Repo:
        ...
        try:
            ...
            if isinstance(
                e, AssertionError
            ) and "Invalid object name" not in str(e):
                raise
