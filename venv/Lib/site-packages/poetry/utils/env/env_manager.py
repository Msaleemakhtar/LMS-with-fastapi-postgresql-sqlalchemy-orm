...
class EnvManager:
    ...
    def get_python_version(
        ...
        if prefer_active_python:
            ...
            if executable:
                ...
                version = ".".join(
                    str(v) for v in python_patch.split(".")[:precision]
                )
    ...
    def activate(self, python: str) -> Env:
        ...
        base_env_name = self.generate_env_name(
            self._poetry.package.name, str(cwd)
        )
        if envs_file.exists():
            envs = envs_file.read()
    ...
    def get(self, reload: bool = False) -> Env:
        ...
        env_prefix = os.environ.get(
            "VIRTUAL_ENV", os.environ.get("CONDA_PREFIX")
        )
    ...
    def list(self, name: str | None = None) -> list[VirtualEnv]:
        ...
        venv_name = self.generate_env_name(
            name, str(self._poetry.file.path.parent)
        )
        venv_path = self._poetry.config.virtualenvs_path
        env_list = [
            VirtualEnv(p) for p in sorted(venv_path.glob(f"{venv_name}-py*"))
        ]
    ...
    def use_in_project_venv(self) -> bool:
        in_project: bool | None = self._poetry.config.get(
    ...
    def create_venv(
        ...
        if env.is_venv() and not force:
            ...
            if not self._poetry.package.python_constraint.allows(
                current_python
            ):
        ...
        if not supported_python.allows(Version.parse(python_patch)):
            ...
            for suffix in sorted(
                ...
                if self._io.is_debug():
                    self._io.write_error_line(
                        f"<debug>Trying {python_name}</debug>"
                    )
        ...
        if not venv.exists():
            ...
            if force:
                ...
                self._io.write_error_line(
                    f"Virtualenv <c1>{name}</> already exists."
                )
    ...
    def build_venv(
        ...
        if WINDOWS:
            ...
            executable = (
                get_real_windows_path(executable) if executable else None
            )

        executable_str = (
            None if executable is None else executable.resolve().as_posix()
        )
