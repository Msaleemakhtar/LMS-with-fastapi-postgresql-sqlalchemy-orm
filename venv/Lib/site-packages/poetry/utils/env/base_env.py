...
class Env:
    ...
    def __init__(self, path: Path, base: Path | None = None) -> None:
        ...
        bin_dir = (
            "bin" if not self._is_windows or self._is_mingw else "Scripts"
        )
        self._path = path
    ...
    def version_info(self) -> tuple[int, int, int, str, int]:
        version_info: tuple[int, int, int, str, int] = self.marker_env[
            "version_info"
        ]
        return version_info
    ...
    def script_dirs(self) -> list[Path]:
        if self._script_dirs is None:
            ...
            if self.userbase:
                self._script_dirs.append(
                    self.userbase / self._script_dirs[0].name
                )
