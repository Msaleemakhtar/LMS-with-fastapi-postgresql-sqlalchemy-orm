...
class SitePackages:
    def __init__(
        ...
        for path in itertools.chain(
            [self._purelib, self._platlib], self._fallbacks
        ):
            if path not in self._candidates:
                self._candidates.append(path)

        self._writable_candidates = (
            None if not skip_write_checks else self._candidates
        )
    ...
    def make_candidates(
        ...
        candidates = (
            self._candidates if not writable_only else self.writable_candidates
        )
        if path.is_absolute():
    ...
    def find_distribution(
        ...
        for distribution in self.distributions(
            name=name, writable_only=writable_only
        ):
            return distribution
        return None
    ...
    def exists(self, path: Path) -> bool:
        return any(
            ...
            for value in self._path_method_wrapper(
                path, "exists", return_first=False
            )
        )
