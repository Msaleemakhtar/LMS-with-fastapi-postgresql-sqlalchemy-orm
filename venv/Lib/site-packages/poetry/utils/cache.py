...
class FileCache(Generic[T]):
    ...
    def put(self, key: str, value: Any, minutes: int | None = None) -> None:
        ...
        payload: CacheItem[Any] = CacheItem(
            value,
            expires=_expiration(minutes) if minutes is not None else None,
        )
        path = self._path(key)
...
class ArtifactCache:
    ...
    def get_cached_archive_for_git(
        ...
        cache_dir = self.get_cache_directory_for_git(
            url, reference, subdirectory
        )
    ...
    def _get_cached_archive(
        ...
        *,
        ...
        for archive in archives:
            ...
            candidates.append(
                (
                    wheel.get_minimum_supported_index(env.supported_tags),
                    archive,
                ),
            )
