...
class Application(BaseApplication):
    ...
    def create_io(
        ...
        formatter.set_style(
            "c2_dark", Style("default", options=["bold", "dark"])
        )
    ...
    def render_error(self, error: Exception, io: IO) -> None:
        ...
        self.set_solution_provider_repository(
            self._get_solution_provider_repository()
        )
        ...
        if name == "run":
            ...
            for option_name, value in input.options.items():
                if value:
                    ...
                    if option.shortcut:
                        shortcuts = re.split(
                            r"\|-?", option.shortcut.lstrip("-")
                        )
                        shortcuts = [s for s in shortcuts if s]
                        for shortcut in shortcuts:
                            run_input.add_parameter_option(
                                "-" + shortcut.lstrip("-")
                            )
    ...
    def configure_env(
        ...
        assert isinstance(event, ConsoleCommandEvent)
        command = event.command
        if not isinstance(command, EnvCommand) or isinstance(
            command, SelfCommand
        ):
            return
    ...
    @staticmethod
    def configure_installer_for_command(
        command: InstallerCommand, io: IO
    ) -> None:
