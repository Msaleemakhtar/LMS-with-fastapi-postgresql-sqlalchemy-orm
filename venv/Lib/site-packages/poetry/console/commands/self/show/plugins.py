...
class PluginPackage:
    package: Package
    plugins: list[metadata.EntryPoint] = dataclasses.field(
    ...
    def _system_project_handle(self) -> int:
        ...
        for name, info in plugins.items():
            ...
            if info.plugins:
                ...
                provide_line += (
                    f" <info>{count}</info> plugin{pluralize(count)}"
                )
            ...
            if info.application_plugins:
                ...
                provide_line += f" <info>{count}</info> application plugin{pluralize(count)}"
