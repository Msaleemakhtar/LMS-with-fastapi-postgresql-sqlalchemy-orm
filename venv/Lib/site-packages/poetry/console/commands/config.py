...
To remove a repository (repo is a short alias for repositories):
    ...
    def handle(self) -> int:
        ...
        try:
            local_config_file = TOMLFile(
                self.poetry.file.path.parent / "poetry.toml"
            )
        ...
        if not self.argument("value") and not self.option("unset"):
            if setting_key.split(".")[0] in self.LIST_PROHIBITED_SETTINGS:
                raise ValueError(
                    f"Expected a value for {setting_key} setting."
                )

            m = re.match(
                r"^repos?(?:itories)?(?:\.(.+))?", self.argument("key")
            )
    ...
    def _list_configuration(
        ...
        for key, value in sorted(config.items()):
            ...
            if k.startswith("repositories."):
                message = (
                    f"<c1>{k + key}</c1> = <c2>{json.dumps(raw_val)}</c2>"
                )
