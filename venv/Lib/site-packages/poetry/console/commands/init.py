...
class InitCommand(Command):
    name = "init"
    description = "Creates a basic <comment>pyproject.toml</> file in the current directory."
...
"""
    ...
    def handle(self) -> int:
        ...
        if not license:
            license = self.ask(
                self.create_question("License []: ", default="")
            )
        ...
        if not python:
            ...
            default_python = (
                ...
                + EnvManager.get_python_version(
                    precision=2,
                    prefer_active_python=config.get(
...
"""
    ...
    def _determine_requirements(
                ...
                if package and "version" not in constraint:
                    ...
                    if package_constraint is None:
                        (
                            _,
                            package_constraint,
                        ) = self._find_best_version_for_package(package)
        ...
        for requirement in self._parse_requirements(requires):
            if (
                "git" in requirement
                or "url" in requirement
                or "path" in requirement
            ):
