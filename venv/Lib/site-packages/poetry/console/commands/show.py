...
lists all packages available."""
    ...
    def handle(self) -> int:
        ...
        if package:
            return self._display_single_package_information(
                package, locked_repo
            )
    ...
    def _display_packages_information(
        ...
        from poetry.repositories.installed_repository import (
            InstalledRepository,
        )
        ...
        locked_packages = locked_repository.packages
        pool = RepositoryPool.from_packages(
            locked_packages, self.poetry.config
        )
        solver = Solver(
            root,
            pool=pool,
        ...
        for locked in locked_packages:
            ...
            if show_latest:
                ...
                if (
                    not self.option("outdated")
                    or update_status != "up-to-date"
                ):
        ...
        write_latest = (
            name_length + version_length + latest_length + 3 <= width
        )
    ...
    def display_package_tree(
        ...
        if why_package is not None:
            dependencies = [
                p for p in package.requires if p.name == why_package.name
            ]
        else:
        ...
        if package.is_direct_origin():
            for dep in requires:
                if (
                    ...
                    and dep.source_type == package.source_type
                ):
